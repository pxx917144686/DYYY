...
// 子分组折叠兼容：优先取表格当前可见条目（子分组标题 + 已展开子分组），否则回退完整分区。
static NSArray<DYYYSettingItem *> *DYYYVisibleSettingItems(UITableView *tableView,
                                                           NSInteger section,
                                                           NSArray<NSArray<DYYYSettingItem *> *> *settingSections) {
    id dataSource = tableView.dataSource;
    if ([dataSource isKindOfClass:[DYYYSettingViewController class]]) {
        NSArray<DYYYSettingItem *> *visible = [(DYYYSettingViewController *)dataSource visibleItemsForSection:section];
        if (visible) {
            return visible;
        }
    }
    if (section >= 0 && section < (NSInteger)settingSections.count) {
...
- (void)handleSpecialSwitchTypes:(DYYYSettingItem *)item 
    ...
    // 毛玻璃与全部液态玻璃互斥：开一个关另一个（双向对称）。
    // 运行时若两者同时为开，液态玻璃优先生效（DYYYLegacyCommentBlurActive 会放行）。
    if ([sender isOn]) {
        NSArray<NSString *> *liquidGlassKeys = @[
            @"DYYYCommentGlass",
            @"DYYYCommentGlassClear",
            @"DYYYInnerNotiGlass",
            @"DYYYInnerNotiGlassClear",
            @"DYYYGlassTabBar",
            @"DYYYGlassTabBarClear",
            @"DYYYSharePanelGlass",
            @"DYYYSharePanelGlassClear",
            @"DYYYisEnableModern"
        ];
        if ([item.key isEqualToString:@"DYYYisEnableCommentBlur"]) {
            for (NSString *liquidKey in liquidGlassKeys) {
                [self syncLinkedSwitch:liquidKey enabled:NO tableView:tableView settingSections:settingSections];
            }
        } else if ([liquidGlassKeys containsObject:item.key]) {
            [self syncLinkedSwitch:@"DYYYisEnableCommentBlur" enabled:NO tableView:tableView settingSections:settingSections];
        }
    }
...
- (void)updateMutuallyExclusiveSwitches:(NSInteger)section 
    ...
    NSArray<DYYYSettingItem *> *sectionItems = DYYYVisibleSettingItems(tableView, section, settingSections);
...
- (void)updateSubSwitchesInSection:(NSInteger)section 
    ...
    @try {
        ...
        for (NSUInteger row = 0; row < sectionItems.count; row++) {
            ...
            // 子分组标题行 key 为 nil，跳过（NSArray containsObject:nil 会抛异常）
            if (item.key && [keys containsObject:item.key]) {
                NSIndexPath *cellPath = [NSIndexPath indexPathForRow:row inSection:section];
                UITableViewCell *cell = [tableView cellForRowAtIndexPath:cellPath];
...
- (void)syncLinkedSwitch:(NSString *)targetKey
    ...
    @try {
        ...
        for (NSInteger sec = 0; sec < (NSInteger)settingSections.count; sec++) {
            NSArray<DYYYSettingItem *> *items = DYYYVisibleSettingItems(tableView, sec, settingSections);
...
- (void)updateAreaSubSwitchesUI:(NSInteger)section 
    ...
    for (NSUInteger row = 0; row < sectionItems.count; row++) {
        ...
        // 子分组标题行 key 为 nil，跳过（NSArray containsObject:nil 会抛异常）
        if (item.key && [areaSubKeys containsObject:item.key]) {
            NSIndexPath *cellPath = [NSIndexPath indexPathForRow:row inSection:section];
            UITableViewCell *cell = [tableView cellForRowAtIndexPath:cellPath];
