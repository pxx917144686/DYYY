...
+ (void)takeScreenshot {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!topVC) {
            [DYYYManager showToast:@"无法获取当前界面"];
            return;
        }
...
+ (void)presentShareSheetWithImage:(UIImage *)image fromView:(UIView *)sourceView {
    if (!image) {
        [DYYYManager showToast:@"截图处理失败，无法分享"];
        return;
    }
    ...
    if (!presentingController) {
        ...
        [DYYYManager showToast:@"截图已保存到相册"];
        return;
    }
...
+ (void)takeScreenshotOfView:(UIView *)view {
    if (!view) {
        [DYYYManager showToast:@"无法获取目标视图"];
        return;
    }
...
+ (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (error) {
            [DYYYManager showToast:[NSString stringWithFormat:@"截图保存失败: %@", error.localizedDescription]];
        } else {
            // 保存成功，播放系统截图声音和触感反馈
