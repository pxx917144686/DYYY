...
@interface DYYYFLEXDoKitNetworkViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation DYYYFLEXDoKitNetworkViewController
...
- (void)refreshData {
    NSArray *allRequests = [[DYYYFLEXDoKitNetworkMonitor sharedInstance] networkRequests];
...
- (void)clearLogs {
    [[[DYYYFLEXDoKitNetworkMonitor sharedInstance] networkRequests] removeAllObjects];
    [self refreshData];
}
...
- (void)showMockSettings {
    ...
    DYYYFLEXDoKitNetworkMonitor *monitor = [DYYYFLEXDoKitNetworkMonitor sharedInstance];
...
- (void)showAddMockRule {
    ...
    UIAlertAction *addAction = [UIAlertAction actionWithTitle:@"添加"
        ...
        if (url.length > 0 && responseData.length > 0) {
            ...
            [[DYYYFLEXDoKitNetworkMonitor sharedInstance] addMockRule:rule];
        }
    }];
...
- (void)showSlowNetworkSettings {
    ...
    UIAlertAction *setDelayAction = [UIAlertAction actionWithTitle:@"设置延迟"
        ...
        [[DYYYFLEXDoKitNetworkMonitor sharedInstance] simulateSlowNetwork:delay];
    }];
    
    UIAlertAction *simulateErrorAction = [UIAlertAction actionWithTitle:@"模拟网络错误"
                                                                 style:UIAlertActionStyleDestructive
                                                               handler:^(UIAlertAction *action) {
        [[DYYYFLEXDoKitNetworkMonitor sharedInstance] simulateNetworkError];
    }];
    
    UIAlertAction *resetAction = [UIAlertAction actionWithTitle:@"重置"
                                                         style:UIAlertActionStyleDefault
                                                       handler:^(UIAlertAction *action) {
        [[DYYYFLEXDoKitNetworkMonitor sharedInstance] resetNetworkSimulation];
    }];
