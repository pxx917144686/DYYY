...
@class DYYYFLEXPropertyAttributes, DYYYFLEXMethodBase;


#pragma mark DYYYFLEXProperty
@interface DYYYFLEXProperty : NSObject
...
/// 使用给定的名称和属性构造一个新属性。
+ (instancetype)propertyWithName:(NSString *)name attributes:(DYYYFLEXPropertyAttributes *)attributes;
...
/// 属性的特性。
@property (nonatomic          ) DYYYFLEXPropertyAttributes *attributes;
...
/// 调用 -getValue: 并将该值传递到
/// -[DYYYFLEXRuntimeUtility potentiallyUnwrapBoxedPointer:type:]
/// 并返回结果。
///
/// 如果这是一个类属性，你必须传递类对象。
- (id)getPotentiallyUnboxedValue:(id)target;

/// 无论 \c DYYYFLEXProperty 实例如何初始化，都可以安全使用。
///
...
- (DYYYFLEXMethodBase *)getterWithImplementation:(IMP)implementation;
/// @return 具有给定实现的属性的setter。
/// @discussion 考虑使用 \c FLEXPropertySetter 宏。
- (DYYYFLEXMethodBase *)setterWithImplementation:(IMP)implementation;

#pragma mark DYYYFLEXMethod 属性 getter / setter 宏
// 在大多数情况下比自己使用上述方法更容易

/// 接受一个 \c DYYYFLEXProperty 和一个类型（例如 \c NSUInteger 或 \c id）并
/// 使用 \c DYYYFLEXProperty 的 \c attribute 的 \c backingIvarName 获取Ivar。
#define FLEXPropertyGetter(DYYYFLEXProperty, type) [DYYYFLEXProperty \
    getterWithImplementation:imp_implementationWithBlock(^(id self) { \
        return *(type *)[self getIvarAddressByName:DYYYFLEXProperty.attributes.backingIvar]; \
    }) \
];
/// 接受一个 \c DYYYFLEXProperty 和一个类型（例如 \c NSUInteger 或 \c id）并
/// 使用 \c DYYYFLEXProperty 的 \c attribute 的 \c backingIvarName 设置Ivar。
#define FLEXPropertySetter(DYYYFLEXProperty, type) [DYYYFLEXProperty \
    setterWithImplementation:imp_implementationWithBlock(^(id self, type value) { \
        [self setIvarByName:DYYYFLEXProperty.attributes.backingIvar value:&value size:sizeof(type)]; \
    }) \
];
/// 接受一个 \c DYYYFLEXProperty、一个类型（例如 \c NSUInteger 或 \c id）和一个Ivar名称字符串来获取Ivar。
#define FLEXPropertyGetterWithIvar(DYYYFLEXProperty, ivarName, type) [DYYYFLEXProperty \
    getterWithImplementation:imp_implementationWithBlock(^(id self) { \
        return *(type *)[self getIvarAddressByName:ivarName]; \
    }) \
];
/// 接受一个 \c DYYYFLEXProperty、一个类型（例如 \c NSUInteger 或 \c id）和一个Ivar名称字符串来设置Ivar。
#define FLEXPropertySetterWithIvar(DYYYFLEXProperty, ivarName, type) [DYYYFLEXProperty \
    setterWithImplementation:imp_implementationWithBlock(^(id self, type value) { \
