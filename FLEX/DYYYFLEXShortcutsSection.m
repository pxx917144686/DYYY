...
@interface DYYYFLEXShortcutsSection ()
...
@property (nonatomic, copy) NSArray<id<DYYYFLEXShortcut>> *shortcuts;
@property (nonatomic, readonly) NSArray<id<DYYYFLEXShortcut>> *allShortcuts;
@end

@implementation DYYYFLEXShortcutsSection
...
+ (instancetype)forObject:(id)objectOrClass additionalRows:(NSArray *)toPrepend {
    NSArray *rows = [DYYYFLEXShortcutsFactory shortcutsForObjectOrClass:objectOrClass];
...
- (id)initWithObject:object rows:(NSArray *)rows isNewSection:(BOOL)newSection {
    ...
    if (self) {
        ...
        _allShortcuts = [rows flex_mapped:^id(id obj, NSUInteger idx) {
            return [DYYYFLEXShortcut shortcutFor:obj];
        }];
        _numberOfLines = 1;
...
- (void)reloadData {
    [DYYYFLEXObjectExplorer configureDefaultsForItems:self.allShortcuts];
    
    // 从快捷方式生成所有（副）标题
    if (self.allShortcuts) {
        self.allTitles = [self.allShortcuts flex_mapped:^id(id<DYYYFLEXShortcut> s, NSUInteger idx) {
            return [s titleWith:self.object];
        }];
        self.allSubtitles = [self.allShortcuts flex_mapped:^id(id<DYYYFLEXShortcut> s, NSUInteger idx) {
...
- (void (^)(__kindof UIViewController *))didPressInfoButtonAction:(NSInteger)row {
    id<DYYYFLEXShortcut> shortcut = self.shortcuts[row];
...
- (void)configureCell:(__kindof DYYYFLEXTableViewCell *)cell forRow:(NSInteger)row {
...
@interface DYYYFLEXShortcutsFactory () {
    BOOL _append, _prepend, _replace, _notInstance;
    NSArray<NSString *> *_properties, *_ivars, *_methods;
}
@end

#define NewAndSet(ivar) ({ DYYYFLEXShortcutsFactory *r = [self sharedFactory]; r->ivar = YES; r; })
...
@implementation DYYYFLEXShortcutsFactory {
    // 类桶
...
+ (instancetype)sharedFactory {
    static DYYYFLEXShortcutsFactory *shared = nil;
...
- (NSArray<id<FLEXRuntimeMetadata>> *)shortcutsForObjectOrClass:(id)objectOrClass {
    ...
    [DYYYFLEXObjectExplorer configureDefaultsForItems:shortcuts];
    return shortcuts;
}

+ (DYYYFLEXShortcutsFactory *)append {
    return NewAndSet(_append);
}

+ (DYYYFLEXShortcutsFactory *)prepend {
    return NewAndSet(_prepend);
}

+ (DYYYFLEXShortcutsFactory *)replace {
...
- (DYYYFLEXShortcutsFactory *)class {
    ...
    return ^(Class cls) {
        ...
        if (self->_properties) {
            NSArray *items = [self->_properties flex_mapped:^id(NSString *name, NSUInteger idx) {
                return [DYYYFLEXProperty named:name onClass:clsForMetadata];
            }];
        ...
        if (self->_methods) {
            NSArray *items = [self->_methods flex_mapped:^id(NSString *name, NSUInteger idx) {
                return [DYYYFLEXMethod selector:NSSelectorFromString(name) class:clsForMetadata];
            }];
        ...
        if (self->_ivars) {
            ...
            NSArray *items = [self->_ivars flex_mapped:^id(NSString *name, NSUInteger idx) {
                return [DYYYFLEXIvar named:name onClass:clsForMetadata];
            }];
