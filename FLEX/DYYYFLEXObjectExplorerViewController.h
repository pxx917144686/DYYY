//
//  DYYYFLEXObjectExplorerViewController.h
//  Flipboard
//
...
@class DYYYFLEXTableViewSection;

NS_ASSUME_NONNULL_BEGIN

/// 一个显示对象或类信息的类。
///
/// 探索器视图控制器使用 \c DYYYFLEXObjectExplorer 提供对象的描述，
/// 并列出它的属性、实例变量、方法及其父类。
/// 在描述下方和属性之前，某些类（如UIViews）会显示一些快捷方式。
/// 在最底部，有一个选项可以查看其他引用正在探索的对象的对象列表。
@interface DYYYFLEXObjectExplorerViewController : DYYYFLEXFilteringTableViewController

/// 为此对象使用默认的 \c DYYYFLEXShortcutsSection 作为自定义部分。
+ (instancetype)exploringObject:(id)objectOrClass;
/// 除非您提供自定义部分，否则没有自定义部分。
+ (instancetype)exploringObject:(id)objectOrClass customSection:(nullable DYYYFLEXTableViewSection *)customSection;
/// 除非您提供一些自定义部分，否则没有自定义部分。
+ (instancetype)exploringObject:(id)objectOrClass
                 customSections:(nullable NSArray<DYYYFLEXTableViewSection *> *)customSections;

/// 正在探索的对象，可能是类的实例或类本身。
@property (nonatomic, readonly) id object;
/// 该对象为探索器视图控制器提供对象的元数据。
@property (nonatomic, readonly) DYYYFLEXObjectExplorer *explorer;
