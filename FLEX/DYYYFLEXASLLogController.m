...
@interface DYYYFLEXASLLogController ()

@property (nonatomic, readonly) void (^updateHandler)(NSArray<DYYYFLEXSystemLogMessage *> *);
...
@implementation DYYYFLEXASLLogController
...
- (id)initWithUpdateHandler:(void(^)(NSArray<DYYYFLEXSystemLogMessage *> *newMessages))newMessagesHandler {
...
- (void)updateLogMessages {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray<DYYYFLEXSystemLogMessage *> *newMessages;
        @synchronized (self) {
            ...
            for (DYYYFLEXSystemLogMessage *message in newMessages) {
...
- (NSArray<DYYYFLEXSystemLogMessage *> *)newLogMessagesForCurrentProcess {
    ...
    NSMutableArray<DYYYFLEXSystemLogMessage *> *newMessages = [NSMutableArray new];
    ...
    while ((aslMessage = asl_next(response))) {
        ...
        if (![self.logMessageIdentifiers containsIndex:messageID]) {
            [newMessages addObject:[DYYYFLEXSystemLogMessage logMessageFromASLMessage:aslMessage]];
        }
    }
...
- (NSArray<DYYYFLEXSystemLogMessage *> *)allLogMessagesForCurrentProcess {
    aslresponse response = [self ASLMessageListForCurrentProcess];
    aslmsg aslMessage = NULL;

    NSMutableArray<DYYYFLEXSystemLogMessage *> *logMessages = [NSMutableArray new];
    while ((aslMessage = asl_next(response))) {
        [logMessages addObject:[DYYYFLEXSystemLogMessage logMessageFromASLMessage:aslMessage]];
    }
    asl_release(response);
