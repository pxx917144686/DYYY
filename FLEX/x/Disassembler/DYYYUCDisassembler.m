...
@implementation DYYYUCDisasmOperand
@end

@implementation DYYYUCDisasmInstruction
@end

@implementation DYYYUCDisasmResult
@end

@implementation DYYYUCBasicBlock
- (instancetype)init {
    self = [super init];
    if (self) {
...
@implementation DYYYUCFunction
// 释放时清空 basicBlocks，间接触发 DYYYUCBasicBlock dealloc 打破循环
- (void)dealloc {
...
#pragma mark - DYYYUCDisassembler

@implementation DYYYUCDisassembler

+ (instancetype)sharedInstance {
    static DYYYUCDisassembler *instance = nil;
...
- (void)initCapstone {
    ...
    if (err != CS_ERR_OK) {
        NSLog(@"[DYYYUCDisassembler] cs_open failed: %s", cs_strerror(err));
        return;
    }
    
    cs_err err2 = cs_option(g_capstoneHandle, CS_OPT_DETAIL, CS_OPT_ON);
    if (err2 != CS_ERR_OK) {
        NSLog(@"[DYYYUCDisassembler] cs_option DETAIL failed: %s", cs_strerror(err2));
    }
    
    cs_err err3 = cs_option(g_capstoneHandle, CS_OPT_SKIPDATA, CS_OPT_ON);
    if (err3 != CS_ERR_OK) {
        NSLog(@"[DYYYUCDisassembler] cs_option SKIPDATA failed: %s", cs_strerror(err3));
    }
    
    g_disasmQueue = dispatch_queue_create("com.ucdisassembler.queue", DISPATCH_QUEUE_SERIAL);
    
    g_capstoneInitialized = YES;
    NSLog(@"[DYYYUCDisassembler] Capstone AArch64 engine initialized (Pro mode)");
}

- (void)dealloc {
    // 全局句柄由单例管理，非单例实例释放时不关闭
    // 避免 [[DYYYUCDisassembler alloc] init] 释放后影响 sharedInstance
}

#pragma mark - 公共反汇编方法

- (DYYYUCDisasmResult *)disassembleAtAddress:(uint64_t)address size:(NSUInteger)size {
    if (address == 0 || size == 0) return nil;
    
    if (![DYYYUCDisassembler isAddressReadable:address size:MIN(size, 4)]) {
        return nil;
    }
    
    DYYYUCDisasmResult *result = [[DYYYUCDisasmResult alloc] init];
...
- (DYYYUCDisasmResult *)disassembleMethod:(Method)method {
    ...
    DYYYUCFunction *func = [self analyzeFunctionAtAddress:addr maxSize:65536];
    ...
    if (func && func.instructions.count > 0) {
        ...
        for (DYYYUCDisasmInstruction *insn in func.instructions) {
            addrMap[@(insn.address)] = insn;
        }
        result.addressMap = addrMap;
        ...
        DYYYUCDisasmResult *fallback = [self disassembleAtAddress:addr size:actualSize];
        if (fallback) {
...
- (DYYYUCDisasmResult *)disassembleClass:(Class)cls selector:(SEL)selector isClassMethod:(BOOL)isClassMethod {
...
- (NSArray<DYYYUCDisasmInstruction *> *)disassembleWithCapstone:(uint64_t)address size:(NSUInteger)size {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        DYYYUCDisasmInstruction *di = [[DYYYUCDisasmInstruction alloc] init];
...
- (NSArray<DYYYUCDisasmOperand *> *)parseOperands:(cs_insn *)insn {
    ...
    for (int i = 0; i < arm64->op_count; i++) {
        ...
        DYYYUCDisasmOperand *operand = [[DYYYUCDisasmOperand alloc] init];
...
- (void)classifyInstruction:(DYYYUCDisasmInstruction *)di {
    ...
    if (di.isBranch && di.operandList.count > 0) {
        DYYYUCDisasmOperand *targetOp = di.operandList.lastObject;
    ...
    if (([mnemonic isEqualToString:@"stp"] || [mnemonic isEqualToString:@"stur"]) &&
        ...
        DYYYUCDisasmOperand *op0 = di.operandList[0];
...
- (NSArray<DYYYUCDisasmInstruction *> *)hexDumpAtAddress:(uint64_t)address size:(NSUInteger)size {
...
- (nullable DYYYUCFunction *)analyzeFunctionAtAddress:(uint64_t)address maxSize:(NSUInteger)maxSize {
    if (address == 0 || maxSize == 0) return nil;
    
    NSArray<DYYYUCDisasmInstruction *> *insns = [self disassembleWithCapstone:address size:maxSize];
    ...
    for (NSUInteger i = 0; i < insns.count; i++) {
        DYYYUCDisasmInstruction *insn = insns[i];
        ...
        if (foundEnd && i > 0) {
            DYYYUCDisasmInstruction *prev = insns[i - 1];
    ...
    DYYYUCFunction *func = [[DYYYUCFunction alloc] init];
    func.startAddress = funcStart;
    func.endAddress = funcEnd;
    func.size = funcSize;
    func.name = [DYYYUCDisassembler symbolNameAtAddress:funcStart] ?:
...
- (nullable NSArray<DYYYUCBasicBlock *> *)buildCFG:(NSArray<DYYYUCDisasmInstruction *> *)instructions {
    if (instructions.count == 0) return nil;
    
    NSMutableDictionary<NSNumber *, DYYYUCDisasmInstruction *> *addrMap = [NSMutableDictionary dictionary];
    NSMutableSet<NSNumber *> *blockStarts = [NSMutableSet set];

    for (DYYYUCDisasmInstruction *insn in instructions) {
        addrMap[@(insn.address)] = insn;
    }
    ...
    for (DYYYUCDisasmInstruction *insn in instructions) {
        if (insn.isBranch) {
            ...
            if (idx < instructions.count - 1) {
                DYYYUCDisasmInstruction *next = instructions[idx + 1];
    ...
    NSMutableArray<DYYYUCBasicBlock *> *blocks = [NSMutableArray array];
    NSMutableDictionary<NSNumber *, DYYYUCBasicBlock *> *blockMap = [NSMutableDictionary dictionary];
    
    for (NSUInteger i = 0; i < sortedStarts.count; i++) {
        uint64_t startAddr = [sortedStarts[i] unsignedLongLongValue];
        DYYYUCDisasmInstruction *startInsn = addrMap[@(startAddr)];
        if (!startInsn) continue;
        
        NSUInteger startIdx = [instructions indexOfObject:startInsn];
        if (startIdx == NSNotFound) continue;
        
        DYYYUCBasicBlock *block = [[DYYYUCBasicBlock alloc] init];
        ...
        for (NSUInteger j = startIdx; j < instructions.count; j++) {
            DYYYUCDisasmInstruction *insn = instructions[j];
            ...
            if (j + 1 < instructions.count) {
                DYYYUCDisasmInstruction *next = instructions[j + 1];
    ...
    for (DYYYUCBasicBlock *block in blocks) {
        DYYYUCDisasmInstruction *lastInsn = block.instructions.lastObject;
        
        if (lastInsn.isConditionalBranch && lastInsn.branchTarget > 0) {
            DYYYUCBasicBlock *targetBlock = blockMap[@(lastInsn.branchTarget)];
            if (targetBlock) {
            ...
            if (idx < instructions.count - 1) {
                DYYYUCDisasmInstruction *next = instructions[idx + 1];
                DYYYUCBasicBlock *fallthrough = blockMap[@(next.address)];
...
- (nullable NSArray<DYYYUCFunction *> *)scanFunctionsInRange:(uint64_t)start size:(NSUInteger)size {
    if (start == 0 || size == 0) return nil;
    
    NSMutableArray<DYYYUCFunction *> *functions = [NSMutableArray array];
    ...
    while (addr < end) {
        ...
        if (isPrologue && ![seenAddresses containsObject:@(addr)]) {
            DYYYUCFunction *func = [self analyzeFunctionAtAddress:addr maxSize:65536];
            if (func) {
                func.name = [DYYYUCDisassembler symbolNameAtAddress:addr] ?:
...
- (void)analyzeAndAnnotateResult:(DYYYUCDisasmResult *)result {
    ...
    for (DYYYUCDisasmInstruction *insn in result.instructions) {
        ...
        if (insn.isCall) {
            ...
            if (insn.branchTarget > 0) {
                targetName = [DYYYUCDisassembler symbolNameAtAddress:insn.branchTarget];
                if (!targetName) {
                    targetName = [DYYYUCDisassembler objcMethodNameAtAddress:insn.branchTarget];
                }
                if (targetName) {
...
- (nullable NSString *)findStringReference:(DYYYUCDisasmInstruction *)insn {
    ...
    DYYYUCDisasmOperand *op2 = insn.operandList[1];
    ...
    return [DYYYUCDisassembler stringAtAddress:strAddr maxLength:256];
}

- (nullable NSString *)findObjcClassRef:(DYYYUCDisasmInstruction *)insn {
    return nil;
}
...
+ (DYYYUCSymbolInfo *)symbolInfoAtAddress:(uint64_t)address {
    Dl_info info;
    if (dladdr((void *)address, &info)) {
        if (info.dli_sname) {
            ...
            DYYYUCSymbolInfo *sym = [DYYYUCSymbolInfo infoWithAddress:address name:name type:@"function"];
            sym.imageName = image;
            return sym;
        }
    }
