...
#import "DYYYCDZipWriter.h"
#import "DYYYCDSwiftDumper.h"
...
#pragma mark - DYYYCDClassInfo

@implementation DYYYCDClassInfo
+ (instancetype)infoForClass:(Class)cls {
    if (!cls) return nil;
    
    DYYYCDClassInfo *info = [[DYYYCDClassInfo alloc] init];
...
static NSString *CDHeaderForClass(Class cls, NSString *imageName) {
    @try {
        ...
        NSLog(@"[DYYYCDHeaderDumper] Exception: %@", e);
        return nil;
    }
}
...
@implementation DYYYCDHeaderDumper
            ...
            if (imageIndex >= images.count) {
                ...
                @try {
                    ...
                    for (unsigned int i = 0; i < protocolCount; i++) {
                        ...
                        if (!writeOk) {
                            NSLog(@"[DYYYCDHeaderDumper] 写入文件失败: %@, error: %@", path, writeError);
                        }
                ...
                NSArray<NSString *> *swiftFiles = [DYYYCDSwiftDumper dumpSwiftInterfacesAtRootDir:rootDir progress:^(CGFloat swiftProgress, NSString *text) {
                ...
                if (!writeOk) {
                    NSLog(@"[DYYYCDHeaderDumper] 写入文件失败: %@, error: %@", indexPath, writeError);
                }
                ...
                if (writtenFiles.count == 0) {
                    ...
                    if (!writeOk) {
                        NSLog(@"[DYYYCDHeaderDumper] 写入文件失败: %@, error: %@", emptyPath, writeError);
                    }
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    BOOL ok = [DYYYCDZipWriter createZipAtPath:zipPath
                                                    rootDir:rootDir
                                                     files:writtenFiles
                    ...
                    if (!ok || zipError) {
                        ...
                        if (!writeOk) {
                            NSLog(@"[DYYYCDHeaderDumper] 写入文件失败: %@, error: %@", errorPath, writeError);
                        }
                        if (![writtenFiles containsObject:errorPath]) [writtenFiles addObject:errorPath];

                        zipError = nil;
                        ok = [DYYYCDZipWriter createZipAtPath:zipPath
                                                   rootDir:rootDir
                                                     files:writtenFiles
                                                  progress:nil
                                                     error:&zipError];
...
+ (DYYYCDClassInfo *)classInfoForName:(NSString *)className {
    ...
    return [DYYYCDClassInfo infoForClass:cls];
}
