#import "DYYYUCClassSearchViewController.h"
...
@interface DYYYUCClassSearchViewController () <UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
...
@implementation DYYYUCClassSearchViewController

+ (instancetype)searchViewControllerWithMode:(UCClassSearchMode)mode {
    DYYYUCClassSearchViewController *vc = [[DYYYUCClassSearchViewController alloc] init];
    vc.searchMode = mode;
    return vc;
}
...
- (void)loadClassNames {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        NSArray<NSString *> *names = [DYYYUCClassDumpTool allClassNames];
...
- (void)openClassHeader:(NSString *)className {
    ...
    if (self.searchMode == UCClassSearchModeDisassembler) {
        ...
        DYYYUCClassHeaderDetailViewController *detailVC = [[DYYYUCClassHeaderDetailViewController alloc]
...
- (void)showMethodListForClass:(NSString *)className isClassMethod:(BOOL)isClassMethod {
    ...
    DYYYUCMethodListViewController *methodListVC = [[DYYYUCMethodListViewController alloc]
