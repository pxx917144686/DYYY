//
//  DYYYFLEXHierarchyTableViewController.m
//  Flipboard
//
...
#import "DYYYFLEXHierarchyTableViewCell.h"
...
@interface DYYYFLEXHierarchyTableViewController ()
...
@implementation DYYYFLEXHierarchyTableViewController
...
+ (NSArray<UIView *> *)allViewsInHierarchy:(NSArray<UIWindow *> *)windows {
    return [windows flex_flatmapped:^id(UIWindow *window, NSUInteger idx) {
        if (![window isKindOfClass:[DYYYFLEXWindow class]]) {
...
- (void)updateDisplayedViews {
    ...
    if (self.searchText.length) {
        self.displayedViews = [candidateViews filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UIView *candidateView, NSDictionary<NSString *, id> *bindings) {
            NSString *title = [DYYYFLEXUtility descriptionForView:candidateView includingFrame:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DYYYFLEXHierarchyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (!cell) {
        cell = [[DYYYFLEXHierarchyTableViewCell alloc] initWithReuseIdentifier:CellIdentifier];
    }
    
    UIView *view = self.displayedViews[indexPath.row];

    cell.textLabel.text = [DYYYFLEXUtility descriptionForView:view includingFrame:NO];
    cell.detailTextLabel.text = [DYYYFLEXUtility detailDescriptionForView:view];
    cell.randomColorTag = [DYYYFLEXUtility consistentRandomColorForObject:view];
    ...
    if (view.isHidden || view.alpha < 0.01) {
        cell.textLabel.textColor = DYYYFLEXColor.deemphasizedTextColor;
        cell.detailTextLabel.textColor = DYYYFLEXColor.deemphasizedTextColor;
    } else {
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    ...
    DYYYFLEXObjectExplorerViewController *viewExplorer = [DYYYFLEXObjectExplorerFactory explorerViewControllerForObject:drillInView];
