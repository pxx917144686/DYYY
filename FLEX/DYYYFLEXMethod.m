...
- (id)initWithMethod:(Method)method isInstanceMethod:(BOOL)isInstanceMethod {
    ...
    if (self) {
        ...
        if ([DYYYFLEXTypeEncodingParser methodTypeEncodingSupported:_signatureString cleaned:&cleanSig]) {
...
- (NSString *)prettyName {
    ...
    NSString *readableReturnType = [DYYYFLEXRuntimeUtility readableTypeForEncoding:@(self.signature.methodReturnType ?: "")];
...
- (NSArray *)prettyArgumentComponents {
    ...
    for (NSUInteger argIndex = 2; argIndex < numberOfArguments; argIndex++) {
        ...
        NSString *readableArgType = [DYYYFLEXRuntimeUtility readableTypeForEncoding:@(argType)];
...
@implementation DYYYFLEXMethod (Comparison)
