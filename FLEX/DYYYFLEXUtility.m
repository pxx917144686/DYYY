...
@implementation DYYYFLEXUtility

+ (UIWindow *)appKeyWindow {
    // 首先，检查 UIApplication.keyWindow
    DYYYFLEXWindow *window = (id)UIApplication.sharedApplication.keyWindow;
    if (window) {
    ...
    //
    // 此外，这些显然不全是 FLEXWindows；使用 DYYYFLEXWindow 
    // 是为了我们可以调用 window.previousKeyWindow 而不需要丑陋的类型转换
    for (DYYYFLEXWindow *window in UIApplication.sharedApplication.windows) {
...
+ (UIColor *)hierarchyIndentPatternColor {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        UIImage *indentationPatternImage = DYYYFLEXResources.hierarchyIndentPattern;
...
+ (NSString *)applicationName {
    return DYYYFLEXUtility.applicationImageName.lastPathComponent;
}
