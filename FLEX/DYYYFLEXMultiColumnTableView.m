...
@interface DYYYFLEXMultiColumnTableView () <
...
@implementation DYYYFLEXMultiColumnTableView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.backgroundColor  = DYYYFLEXColor.groupedBackgroundColor;
...
- (void)loadHeaderScrollView {
    ...
    headerScrollView.backgroundColor = DYYYFLEXColor.secondaryGroupedBackgroundColor;
...
- (void)loadContentScrollView {
    ...
    [tableView registerClass:[DYYYFLEXDBQueryRowCell class]
...
- (void)loadLeftView {
    ...
    leftHeader.backgroundColor = DYYYFLEXColor.secondaryBackgroundColor;
...
- (void)loadHeaderData {
    ...
    self.headerViews = [NSArray flex_forEachUpTo:self.numberOfColumns map:^id(NSUInteger column) {
        DYYYFLEXTableColumnHeader *header = [DYYYFLEXTableColumnHeader new];
...
- (void)contentHeaderTap:(UIGestureRecognizer *)gesture {
    ...
    // 重置旧的头部视图
    DYYYFLEXTableColumnHeader *oldHeader = (id)self.headerViews[self.sortColumn];
    oldHeader.sortType = FLEXTableColumnHeaderSortTypeNone;
    
    // 更新新的头部视图
    DYYYFLEXTableColumnHeader *newHeader = (id)self.headerViews[newSortColumn];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // 交替背景颜色
    UIColor *backgroundColor = DYYYFLEXColor.primaryBackgroundColor;
    if (indexPath.row % 2 != 0) {
        backgroundColor = DYYYFLEXColor.secondaryBackgroundColor;
    }
    
    // 左侧表格视图用于行号
    if (tableView == self.leftTableView) {
        DYYYFLEXTableLeftCell *cell = [DYYYFLEXTableLeftCell cellWithTableView:tableView];
    ...
    // 右侧表格视图用于数据
        ...
        DYYYFLEXDBQueryRowCell *cell = [tableView
