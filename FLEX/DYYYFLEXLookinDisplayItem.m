...
@implementation DYYYFLEXLookinDisplayItem
...
- (void)enumerateSelfAndAncestors:(void (^)(DYYYFLEXLookinDisplayItem *, BOOL *))block {
    if (!block) return;
    
    BOOL stop = NO;
    DYYYFLEXLookinDisplayItem *currentItem = self;
...
- (void)enumerateSelfAndChildren:(void (^)(DYYYFLEXLookinDisplayItem *))block {
    if (!block) return;
    
    block(self);
    
    for (DYYYFLEXLookinDisplayItem *child in self.children) {
