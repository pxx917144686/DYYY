...
@implementation DYYYFLEXObjectExplorerDefaults

+ (instancetype)canEdit:(BOOL)editable wantsPreviews:(BOOL)showPreviews {
    DYYYFLEXObjectExplorerDefaults *defaults = [self new];
...
@interface DYYYFLEXObjectExplorer () {
    NSMutableArray<NSArray<DYYYFLEXProperty *> *> *_allProperties;
    NSMutableArray<NSArray<DYYYFLEXProperty *> *> *_allClassProperties;
    NSMutableArray<NSArray<DYYYFLEXIvar *> *> *_allIvars;
    NSMutableArray<NSArray<DYYYFLEXMethod *> *> *_allMethods;
    NSMutableArray<NSArray<DYYYFLEXMethod *> *> *_allClassMethods;
    NSMutableArray<NSArray<DYYYFLEXProtocol *> *> *_allConformedProtocols;
    NSMutableArray<DYYYFLEXStaticMetadata *> *_allInstanceSizes;
    NSMutableArray<DYYYFLEXStaticMetadata *> *_allImageNames;
    NSString *_objectDescription;
}

@property (nonatomic, readonly) id<DYYYFLEXMirror> initialMirror;
@end

@implementation DYYYFLEXObjectExplorer

+ (void)initialize {
    if (self == DYYYFLEXObjectExplorer.class) {
        DYYYFLEXObjectExplorer.reflexAvailable = NSClassFromString(@"FLEXSwiftMirror") != nil;
    }
}
...
- (id<DYYYFLEXMirror>)mirrorForClass:(Class)cls {
    static Class FLEXSwiftMirror = nil;
    
    // 我们应该使用Reflex吗？
    if (FLEXIsSwiftObjectOrClass(cls) && DYYYFLEXObjectExplorer.reflexAvailable) {
        // 如有需要初始化FLEXSwiftMirror类
        if (!FLEXSwiftMirror) {
            FLEXSwiftMirror = NSClassFromString(@"FLEXSwiftMirror");            
        }
        
        return [(id<DYYYFLEXMirror>)[FLEXSwiftMirror alloc] initWithSubject:cls];
    }
    
    // 否则；不是swift对象，或者Reflex不可用
    return [DYYYFLEXMirror reflect:cls];
}
...
+ (void)configureDefaultsForItems:(NSArray<id<FLEXObjectExplorerItem>> *)items {
    ...
    DYYYFLEXObjectExplorerDefaults *mutable = [DYYYFLEXObjectExplorerDefaults
        canEdit:YES wantsPreviews:!hidePreviews
    ];
    DYYYFLEXObjectExplorerDefaults *immutable = [DYYYFLEXObjectExplorerDefaults
...
- (NSString *)objectDescription {
    if (!_objectDescription) {
        // 硬编码UIColor描述
        if ([DYYYFLEXRuntimeUtility safeObject:self.object isKindOfClass:[UIColor class]]) {
        ...
        NSString *description = [DYYYFLEXRuntimeUtility safeDescriptionForObject:self.object];

        if (!description.length) {
            NSString *address = [DYYYFLEXUtility addressOfObject:self.object];
...
- (void)reloadMetadata {
    ...
    NSMutableArray<NSArray<DYYYFLEXProperty *> *> *allProperties = [NSMutableArray new];
    NSMutableArray<NSArray<DYYYFLEXProperty *> *> *allClassProps = [NSMutableArray new];
    NSMutableArray<NSArray<DYYYFLEXMethod *> *> *allMethods = [NSMutableArray new];
    NSMutableArray<NSArray<DYYYFLEXMethod *> *> *allClassMethods = [NSMutableArray new];
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        id<DYYYFLEXMirror> mirror = [self mirrorForClass:cls];
        ...
        // 这将大大减少已开始在这里表现出来的懒惰
        [_allInstanceSizes addObject:[DYYYFLEXStaticMetadata
            style:FLEXStaticMetadataRowStyleKeyValue
            title:@"实例大小" number:@(class_getInstanceSize(cls))
        ]];
        [_allImageNames addObject:[DYYYFLEXStaticMetadata
            style:FLEXStaticMetadataRowStyleDefault
            title:@"图像名称" string:@(class_getImageName(cls) ?: "运行时创建")
        ]];
    }
    
    _classHierarchy = [DYYYFLEXStaticMetadata classHierarchy:self.classHierarchyClasses];
    
    NSArray<NSArray<DYYYFLEXProperty *> *> *properties = allProperties;
    
    // 可能过滤属性支持的实例变量
    if (hideBackingIvars) {
        NSArray<NSArray<DYYYFLEXIvar *> *> *ivars = _allIvars.copy;
        _allIvars = [ivars flex_mapped:^id(NSArray<DYYYFLEXIvar *> *list, NSUInteger idx) {
            // 获取当前层次结构类中所有支持实例变量名称的集合
            NSSet *ivarNames = [NSSet setWithArray:({
                [properties[idx] flex_mapped:^id(DYYYFLEXProperty *p, NSUInteger idx) {
            ...
            // 删除名称在实例变量名称列表中的实例变量
            return [list flex_filtered:^BOOL(DYYYFLEXIvar *ivar, NSUInteger idx) {
    ...
    if (hidePropertyMethods) {
        allMethods = [allMethods flex_mapped:^id(NSArray<DYYYFLEXMethod *> *list, NSUInteger idx) {
            // 获取当前层次结构类中所有属性方法名称的集合
            NSSet *methodNames = [NSSet setWithArray:({
                [properties[idx] flex_flatmapped:^NSArray *(DYYYFLEXProperty *p, NSUInteger idx) {
            ...
            // 删除名称在属性方法名称列表中的方法
            return [list flex_filtered:^BOOL(DYYYFLEXMethod *method, NSUInteger idx) {
    ...
    if (hidePrivateMethods) {
        id methodMapBlock = ^id(NSArray<DYYYFLEXMethod *> *list, NSUInteger idx) {
            // 删除包含下划线的方法
            return [list flex_filtered:^BOOL(DYYYFLEXMethod *method, NSUInteger idx) {
                return ![method.selectorString containsString:@"_"];
            }];
        };
        id propertyMapBlock = ^id(NSArray<DYYYFLEXProperty *> *list, NSUInteger idx) {
            // 删除包含下划线的方法
            return [list flex_filtered:^BOOL(DYYYFLEXProperty *prop, NSUInteger idx) {
    ...
    for (NSArray *matrix in metadatas) {
        for (NSArray *metadataByClass in matrix) {
            [DYYYFLEXObjectExplorer configureDefaultsForItems:metadataByClass];
        }
    }
...
#pragma mark - Reflex
@implementation DYYYFLEXObjectExplorer (Reflex)
