//
//  DYYYFLEXFieldEditorViewController.m
//  FLEX
//
...
@interface DYYYFLEXFieldEditorViewController () <FLEXArgumentInputViewDelegate>

@property (nonatomic, readonly) id<FLEXMetadataAuxiliaryInfo> auxiliaryInfoProvider;
@property (nonatomic) DYYYFLEXProperty *property;
@property (nonatomic) DYYYFLEXIvar *ivar;
...
@implementation DYYYFLEXFieldEditorViewController

#pragma mark - 初始化

+ (instancetype)target:(id)target property:(nonnull DYYYFLEXProperty *)property commitHandler:(void(^)(void))onCommit {
    DYYYFLEXFieldEditorViewController *editor = [self target:target data:property commitHandler:onCommit];
    editor.title = [@"属性: " stringByAppendingString:property.name];
    editor.property = property;
    return editor;
}

+ (instancetype)target:(id)target ivar:(nonnull DYYYFLEXIvar *)ivar commitHandler:(void(^)(void))onCommit {
    DYYYFLEXFieldEditorViewController *editor = [self target:target data:ivar commitHandler:onCommit];
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = DYYYFLEXColor.groupedBackgroundColor;
    ...
    DYYYFLEXArgumentInputView *inputView = [DYYYFLEXArgumentInputViewFactory argumentInputViewForTypeEncoding:self.typeEncoding];
    inputView.inputValue = self.currentValue;
    inputView.delegate = self;
    self.fieldEditorView.argumentInputViews = @[inputView];

    // 不为开关显示"设置"按钮；当开关翻转时我们进行变更
    if ([inputView isKindOfClass:[DYYYFLEXArgumentInputSwitchView class]]) {
...
- (void)actionButtonPressed:(id)sender {
    if (self.property) {
        ...
        NSError *error = nil;
        [DYYYFLEXRuntimeUtility performSelector:setterSelector onObject:self.target withArguments:arguments error:&error];
        if (error) {
            [DYYYFLEXAlert showAlert:@"属性设置失败" message:error.localizedDescription from:self];
            sender = nil; // 不要返回上一页
        }
    } else {
...
- (void)argumentInputViewValueDidChange:(DYYYFLEXArgumentInputView *)argumentInputView {
    if ([argumentInputView isKindOfClass:[DYYYFLEXArgumentInputSwitchView class]]) {
...
- (void)registerAuxiliaryInfo {
    ...
    for (NSString *type in labels) {
        [DYYYFLEXArgumentInputViewFactory registerFieldNames:labels[type] forTypeEncoding:type];
    }
}
