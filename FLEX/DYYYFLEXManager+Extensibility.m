//
//  DYYYFLEXManager+Extensibility.m
//  FLEX
//
...
- (void)registerGlobalEntryWithName:(NSString *)entryName objectFutureBlock:(id (^)(void))objectFutureBlock {
    ...
    DYYYFLEXGlobalsEntry *entry = [DYYYFLEXGlobalsEntry entryWithNameFuture:^NSString *{
        return entryName;
    } viewControllerFuture:^UIViewController *{
        return [DYYYFLEXObjectExplorerFactory explorerViewControllerForObject:objectFutureBlock()];
    }];
...
- (void)registerGlobalEntryWithName:(NSString *)entryName action:(FLEXGlobalsEntryRowAction)rowSelectedAction {
    ...
    entryName = entryName.copy;
    DYYYFLEXGlobalsEntry *entry = [DYYYFLEXGlobalsEntry entryWithNameFuture:^NSString * _Nonnull{
        return entryName;
...
- (void)setSimulatorShortcutsEnabled:(BOOL)simulatorShortcutsEnabled {
    // 直接使用实例方法，不使用KVC
    DYYYFLEXKeyboardShortcutManager *manager = [DYYYFLEXKeyboardShortcutManager sharedManager];
    [manager setEnabled:simulatorShortcutsEnabled];
}

- (BOOL)simulatorShortcutsEnabled {
    // 直接访问属性
    return [DYYYFLEXKeyboardShortcutManager sharedManager].isEnabled;
}
...
- (void)toggleTopViewControllerOfClass:(Class)class {
    ...
    if ([topViewController isKindOfClass:class]) {
        ...
        if ([topViewController isKindOfClass:[UINavigationController class]]) {
            ...
            // 在全新的导航控制器中呈现它
            UIViewController *newVC = [class new];
            UINavigationController *navController = [DYYYFLEXNavigationController withRootViewController:newVC];
