...
@implementation DYYYFLEXDoKitCrashInfo
...
@interface DYYYFLEXDoKitCrashMonitor ()
@property (nonatomic, strong) NSMutableArray<DYYYFLEXDoKitCrashInfo *> *mutableCrashLogs;
...
@implementation DYYYFLEXDoKitCrashMonitor

+ (instancetype)sharedInstance {
    static DYYYFLEXDoKitCrashMonitor *instance = nil;
...
- (NSMutableArray<DYYYFLEXDoKitCrashInfo *> *)crashLogs {
...
static void flexDoKitSignalHandler(int sig) {
    ...
    DYYYFLEXDoKitCrashInfo *crashInfo = [[DYYYFLEXDoKitCrashInfo alloc] init];
    ...
    crashInfo.deviceInfo = [[DYYYFLEXDoKitCrashMonitor sharedInstance] getDeviceInfo];
    
    [[DYYYFLEXDoKitCrashMonitor sharedInstance] saveCrashInfo:crashInfo];
...
static void flexDoKitExceptionHandler(NSException *exception) {
    ...
    // 调用之前的异常处理器
    DYYYFLEXDoKitCrashMonitor *monitor = [DYYYFLEXDoKitCrashMonitor sharedInstance];
    if (monitor.previousExceptionHandler) {
        monitor.previousExceptionHandler(exception);
    }
}

- (void)saveCrashInfo:(DYYYFLEXDoKitCrashInfo *)crashInfo {
...
- (NSString *)exportCrashLogsAsString {
    ...
    for (DYYYFLEXDoKitCrashInfo *crashInfo in self.mutableCrashLogs) {
