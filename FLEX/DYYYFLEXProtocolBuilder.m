...
#import "DYYYFLEXProtocolBuilder.h"
...
@interface DYYYFLEXProtocolBuilder ()
@property (nonatomic) Protocol *workingProtocol;
@property (nonatomic) NSString *name;
@end

@implementation DYYYFLEXProtocolBuilder
...
- (void)addProperty:(DYYYFLEXProperty *)property isRequired:(BOOL)isRequired {
...
- (DYYYFLEXProtocol *)registerProtocol {
    MutationAssertion(@"协议已经注册");
    
    _isRegistered = YES;
    objc_registerProtocol(self.workingProtocol);
    return [DYYYFLEXProtocol protocol:self.workingProtocol];
}
