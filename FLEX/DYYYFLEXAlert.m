...
+ (void)showQuickAlert:(NSString *)title from:(UIViewController *)viewController {
    UIAlertController *alert = [self makeAlert:^(DYYYFLEXAlert *make) {
        make.title(title);
    }];
...
+ (UIAlertController *)make:(FLEXAlertBuilder)block withStyle:(UIAlertControllerStyle)style {
    // 创建警告构建器
    DYYYFLEXAlert *alert = [[self alloc] initWithController:
    ...
    // 添加操作
    for (DYYYFLEXAlertAction *builder in alert._actions) {
...
- (FLEXAlertStringProperty)title {
    return ^DYYYFLEXAlert *(NSString *title) {
...
- (FLEXAlertStringProperty)message {
    return ^DYYYFLEXAlert *(NSString *message) {
...
- (FLEXAlertAddAction)button {
    return ^DYYYFLEXAlertAction *(NSString *title) {
        DYYYFLEXAlertAction *action = DYYYFLEXAlertAction.new.title(title);
...
- (FLEXAlertStringArg)textField {
    return ^DYYYFLEXAlert *(NSString *placeholder) {
...
- (FLEXAlertTextField)configuredTextField {
    return ^DYYYFLEXAlert *(void(^configurationHandler)(UITextField *)) {
...
@implementation DYYYFLEXAlertAction
...
- (FLEXAlertActionBOOLProperty)enabled {
    return ^DYYYFLEXAlertAction *(BOOL enabled) {
        FLEXAlertActionMutationAssertion();
...
- (FLEXAlertActionHandler)handler {
    return ^DYYYFLEXAlertAction *(void(^handler)(NSArray<NSString *> *)) {
        FLEXAlertActionMutationAssertion();
