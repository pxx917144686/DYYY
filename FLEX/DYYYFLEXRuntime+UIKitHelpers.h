...
@class DYYYFLEXObjectExplorerDefaults;

/// 对象浏览器屏幕的模型对象采用此协议
/// 以便响应用户默认设置的更改
@protocol FLEXObjectExplorerItem <NSObject>
/// 当前浏览器设置。在设置更改时设置。
@property (nonatomic) DYYYFLEXObjectExplorerDefaults *defaults;
...
@interface DYYYFLEXProperty (UIKitHelpers) <FLEXRuntimeMetadata> @end
@interface DYYYFLEXIvar (UIKitHelpers) <FLEXRuntimeMetadata> @end
@interface DYYYFLEXMethodBase (UIKitHelpers) <FLEXRuntimeMetadata> @end
@interface DYYYFLEXMethod (UIKitHelpers) <FLEXRuntimeMetadata> @end
@interface DYYYFLEXProtocol (UIKitHelpers) <FLEXRuntimeMetadata> @end
...
/// 以静态键值对信息的形式显示一个小行。
@interface DYYYFLEXStaticMetadata : NSObject <FLEXRuntimeMetadata>
