//
//  DYYYFLEXExplorerViewController.m
//  Flipboard
//
...
#import "DYYYFLEXTabsViewController.h"
#import "DYYYFLEXWindowManagerController.h"
#import "DYYYFLEXViewControllersViewController.h"
#import "NSUserDefaults+FLEX.h"

// x功能模块
#import "x/ClassDump/DYYYUCClassDumpTool.h"
#import "x/ClassDump/DYYYUCClassSearchViewController.h"
#import "x/Disassembler/DYYYUCDisassembler.h"
#import "x/Disassembler/DYYYUCDisasmViewController.h"
#import "x/filza/DYYYUCFilzaTool.h"
#import "x/Decrypt/DYYYUCDecryptTool.h"
#import "x/Decrypt/DYYYDatabaseManager.h"
...
@interface DYYYFLEXExplorerViewController () <FLEXHierarchyDelegate, UIAdaptivePresentationControllerDelegate>
...
@property (nonatomic, readonly) DYYYFLEXWindow *window;
...
@implementation DYYYFLEXExplorerViewController
...
- (void)viewDidLoad {
    ...
    _explorerToolbar = [DYYYFLEXExplorerToolbar new];
...
- (UIViewController *)viewControllerForRotationAndOrientation {
    UIViewController *viewController = DYYYFLEXUtility.appKeyWindow.rootViewController;
...
//    }
    ...
    UIInterfaceOrientationMask supportedOrientations = DYYYFLEXUtility.infoPlistSupportedInterfaceOrientationsMask;
...
- (void)setSelectedView:(UIView *)selectedView {
    if (![_selectedView isEqual:selectedView]) {
        ...
        self.explorerToolbar.selectedViewDescription = [DYYYFLEXUtility
            descriptionForView:selectedView includingFrame:YES
        ];
        self.explorerToolbar.selectedViewOverlayColor = [DYYYFLEXUtility
        ...
        if (selectedView) {
            ...
            UIColor *outlineColor = [DYYYFLEXUtility consistentRandomColorForObject:selectedView];
...
- (void)setupToolbarActions {
    DYYYFLEXExplorerToolbar *toolbar = self.explorerToolbar;
...
- (void)selectButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    [self toggleSelectTool];
}

- (void)hierarchyButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
...
- (void)recentButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    NSAssert(DYYYFLEXTabList.sharedList.activeTab, @"必须有活动标签");
    [self presentViewController:DYYYFLEXTabList.sharedList.activeTab animated:YES completion:nil];
}

- (void)moveButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    [self toggleMoveTool];
}

- (void)globalsButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    [self toggleMenuTool];
}

- (void)closeButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
...
- (void)classdumpButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    // 直接进入类头文件搜索界面
    DYYYUCClassSearchViewController *searchVC = [[DYYYUCClassSearchViewController alloc] init];
...
- (void)exportAllHeaders {
    ...
    [confirmAlert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [self presentViewController:progressAlert animated:YES completion:^{
            [DYYYUCClassDumpTool dumpHeadersZipWithProgress:^(CGFloat progress, NSString *text) {
...
- (void)decryptButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
...
- (void)disassemblerButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    DYYYUCClassSearchViewController *searchVC = [DYYYUCClassSearchViewController
...
- (void)filzaButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    [DYYYUCFilzaTool presentFilzaPanelFromViewController:self];
}

- (void)protectionButtonTapped:(DYYYFLEXExplorerToolbarItem *)sender {
    [DYYYUCAppProtectionTool enableWithSetup];
    [DYYYUCAppProtectionTool presentProtectionPanelFromViewController:self completion:nil];
}
...
- (void)updateButtonStates {
    ...
    if (!self.presentedViewController) {
        toolbar.recentItem.enabled = DYYYFLEXTabList.sharedList.activeTab != nil;
    } else {
...
- (void)handleToolbarDetailsTapGesture:(UITapGestureRecognizer *)tapGR {
    if (tapGR.state == UIGestureRecognizerStateRecognized && self.selectedView) {
        UIViewController *topStackVC = [DYYYFLEXObjectExplorerFactory explorerViewControllerForObject:self.selectedView];
        [self presentViewController:
            [DYYYFLEXNavigationController withRootViewController:topStackVC]
...
- (void)handleToolbarShowTabsGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        [super presentViewController:[[UINavigationController alloc]
            initWithRootViewController:[DYYYFLEXTabsViewController new]
        ] animated:YES completion:nil];
    }
}
...
- (void)handleToolbarWindowManagerGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        [super presentViewController:[DYYYFLEXNavigationController
            withRootViewController:[DYYYFLEXWindowManagerController new]
        ] animated:YES completion:nil];
    }
}
...
- (void)handleToolbarShowViewControllersGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan && self.viewsAtTapPoint.count) {
        ...
        UIViewController *list = [DYYYFLEXViewControllersViewController
            controllersForViews:self.viewsAtTapPoint
        ];
        [self presentViewController:
            [DYYYFLEXNavigationController withRootViewController:list
        ] animated:YES completion:nil];
    }
}
...
- (UIView *)outlineViewForView:(UIView *)view {
    ...
    outlineView.layer.borderColor = [DYYYFLEXUtility consistentRandomColorForObject:view].CGColor;
...
- (NSArray<UIView *> *)viewsAtPoint:(CGPoint)tapPointInWindow skipHiddenViews:(BOOL)skipHidden {
    ...
    for (UIWindow *window in DYYYFLEXUtility.allWindows) {
...
- (UIView *)viewForSelectionAtPoint:(CGPoint)tapPointInWindow {
    ...
    for (UIWindow *window in DYYYFLEXUtility.allWindows.reverseObjectEnumerator) {
...
- (void)toggleViewsToolWithCompletion:(void(^)(void))completion {
    [self toggleToolWithViewControllerProvider:^UINavigationController *{
        if (self.selectedView) {
            return [DYYYFLEXHierarchyViewController
                delegate:self
                viewsAtTap:self.viewsAtTapPoint
                selectedView:self.selectedView
            ];
        } else {
            return [DYYYFLEXHierarchyViewController delegate:self];
        }
...
- (void)toggleMenuTool {
    [self toggleToolWithViewControllerProvider:^UINavigationController *{
        return [DYYYFLEXNavigationController withRootViewController:[DYYYFLEXGlobalsViewController new]];
    } completion:nil];
}
