...
@interface DYYYFLEXWebViewController () <WKNavigationDelegate>
...
@implementation DYYYFLEXWebViewController
...
- (id)initWithText:(NSString *)text {
    ...
    if (self) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSString *escapedText = [DYYYFLEXUtility stringByEscapingHTMLEntitiesInString:text];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction
    ...
    if (navigationAction.navigationType == WKNavigationTypeOther) {
        ...
        DYYYFLEXWebViewController *webVC = [[[self class] alloc] initWithURL:request.URL];
