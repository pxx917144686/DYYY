...
#import "DYYYFLEXBookmarksViewController.h"

@interface DYYYFLEXTabsViewController ()
...
@property (nonatomic, readonly) DYYYFLEXExplorerViewController *corePresenter;
@end

@implementation DYYYFLEXTabsViewController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [DYYYFLEXTabList.sharedList updateSnapshotForActiveTab];
        [self reloadData:NO];
...
- (BOOL)reloadData:(BOOL)trackActiveTabDelta {
    ...
    DYYYFLEXTabList *list = DYYYFLEXTabList.sharedList;
...
- (void)setupEditingBarItems {
    ...
    self.toolbarItems.firstObject.tintColor = DYYYFLEXColor.destructiveColor;
}

- (DYYYFLEXExplorerViewController *)corePresenter {
    // 我们必须由 DYYYFLEXExplorerViewController 呈现，或由
    // 另一个由 DYYYFLEXExplorerViewController 呈现的视图控制器呈现
    DYYYFLEXExplorerViewController *presenter = (id)self.presentingViewController;
    presenter = (id)presenter.presentingViewController ?: presenter;
    NSAssert(
        [presenter isKindOfClass:[DYYYFLEXExplorerViewController class]],
        @"标签视图控制器应该由探索器控制器呈现"
    );
    return presenter;
}
...
- (void)dismissAnimated {
    if (self.presentNewActiveTabOnDismiss) {
        ...
        UIViewController *activeTab = DYYYFLEXTabList.sharedList.activeTab;
        DYYYFLEXExplorerViewController *presenter = self.corePresenter;
...
- (void)toggleEditing {
    ...
    if (self.isEditing) {
        ...
        if (selected.count) {
            // 关闭标签并更新数据源
            [DYYYFLEXTabList.sharedList closeTabsAtIndexes:indexes];
...
- (void)addTabButtonPressed:(UIBarButtonItem *)sender {
    if (DYYYFLEXBookmarkManager.bookmarkCount > 0) {
        [DYYYFLEXAlert makeSheet:^(DYYYFLEXAlert *make) {
            make.title(@"新标签");
            make.button(@"主菜单").handler(^(NSArray<NSString *> *strings) {
                [self addTabAndDismiss:[DYYYFLEXNavigationController
                    withRootViewController:[DYYYFLEXGlobalsViewController new]
                ]];
            });
            make.button(@"从书签中选择").handler(^(NSArray<NSString *> *strings) {
                [self presentViewController:[DYYYFLEXNavigationController
                    withRootViewController:[DYYYFLEXBookmarksViewController new]
...
- (void)closeAll {
    ...
    // 关闭标签并更新数据源
    [DYYYFLEXTabList.sharedList closeAllTabs];
    [self reloadData:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.editing) {
        ...
        self.toolbarItems.lastObject.tintColor = DYYYFLEXColor.destructiveColor;
    } else {
        if (self.activeIndex == indexPath.row && self.corePresenter != self.presentingViewController) {
            ...
            // 情况：选择了不同的标签，
            // 或从 FLEX 工具栏呈现时选择了标签
            DYYYFLEXTabList.sharedList.activeTabIndex = indexPath.row;
            self.presentNewActiveTabOnDismiss = YES;
            [self dismissAnimated];
        }
    }
...
forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 关闭标签并更新数据源
    [DYYYFLEXTabList.sharedList closeTab:self.openTabs[indexPath.row]];
    BOOL activeTabChanged = [self reloadData:YES];
