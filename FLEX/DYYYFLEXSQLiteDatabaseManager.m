...
@interface DYYYFLEXSQLiteDatabaseManager ()
@property (nonatomic) sqlite3 *db;
@property (nonatomic, copy) NSString *path;
@end

@implementation DYYYFLEXSQLiteDatabaseManager
...
#if SQLITE_HAS_CODEC
    NSString *defaultSqliteDatabasePassword = DYYYFLEXManager.sharedManager.defaultSqliteDatabasePassword;
...
- (NSArray<NSString *> *)queryAllColumnsOfTable:(NSString *)tableName {
    ...
    DYYYFLEXSQLResult *results = [self executeStatement:sql];
...
- (DYYYFLEXSQLResult *)executeStatement:(NSString *)sql {
    return [self executeStatement:sql arguments:nil];
}

- (DYYYFLEXSQLResult *)executeStatement:(NSString *)sql arguments:(NSDictionary *)args {
    [self open];
    
    DYYYFLEXSQLResult *result = nil;
    ...
    if ((status = sqlite3_prepare_v2(_db, sql.UTF8String, -1, &pstmt, 0)) == SQLITE_OK) {
        ...
        if (status == SQLITE_DONE) {
            ...
            if (rows.count || columnCount > 0) {
                // 我们执行了 SELECT 查询
                result = _lastResult = [DYYYFLEXSQLResult columns:columns rows:rows];
            } else {
                ...
                result = _lastResult = [DYYYFLEXSQLResult message:message];
            }
        } else {
            // 执行查询时发生错误
