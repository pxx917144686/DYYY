//
//  DYYYFLEXSystemLogViewController.m
//  FLEX
//
...
#import "DYYYFLEXASLLogController.h"
#import "DYYYFLEXOSLogController.h"
#import "DYYYFLEXSystemLogCell.h"
...
@interface DYYYFLEXSystemLogViewController ()

@property (nonatomic, readonly) DYYYFLEXMutableListSection<DYYYFLEXSystemLogMessage *> *logMessages;
...
@implementation DYYYFLEXSystemLogViewController
...
- (void)viewDidLoad {
    ...
    weakify(self)
    id logHandler = ^(NSArray<DYYYFLEXSystemLogMessage *> *newMessages) { strongify(self)
        [self handleUpdateWithNewMessages:newMessages];
    };

    if (FLEXOSLogAvailable() && !FLEXNSLogHookWorks) {
        _logController = [DYYYFLEXOSLogController withUpdateHandler:logHandler];
    } else {
        _logController = [DYYYFLEXASLLogController withUpdateHandler:logHandler];
    }
    ...
    UIBarButtonItem *scrollDown = [UIBarButtonItem
        flex_itemWithImage:DYYYFLEXResources.scrollToBottomIcon
        target:self
...
- (NSArray<DYYYFLEXTableViewSection *> *)makeSections { weakify(self)
    _logMessages = [DYYYFLEXMutableListSection list:@[]
        cellConfiguration:^(DYYYFLEXSystemLogCell *cell, DYYYFLEXSystemLogMessage *message, NSInteger row) {
            strongify(self)
        ...
        } filterMatcher:^BOOL(NSString *filterText, DYYYFLEXSystemLogMessage *message) {
            NSString *displayedText = [DYYYFLEXSystemLogCell displayedTextForLogMessage:message];
            return [displayedText localizedCaseInsensitiveContainsString:filterText];
        }
    ];

    self.logMessages.cellRegistrationMapping = @{
        kFLEXSystemLogCellIdentifier : [DYYYFLEXSystemLogCell class]
    };
...
- (void)handleUpdateWithNewMessages:(NSArray<DYYYFLEXSystemLogMessage *> *)newMessages {
...
- (void)showLogSettings {
    ...
    DYYYFLEXOSLogController *logController = (DYYYFLEXOSLogController *)self.logController;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    DYYYFLEXSystemLogMessage *logMessage = self.logMessages.filteredList[indexPath.row];
    return [DYYYFLEXSystemLogCell preferredHeightForLogMessage:logMessage inWidth:self.tableView.bounds.size.width];
}
