//
//  DYYYFLEXObjectExplorerViewController.m
//  Flipboard
//
...
#import "DYYYFLEXScopeCarousel.h"
#import "DYYYFLEXMetadataSection.h"
#import "DYYYFLEXSingleRowSection.h"
#import "DYYYFLEXShortcutsSection.h"
#import "NSUserDefaults+FLEX.h"
#import "x/ClassDump/DYYYUCClassHeaderDetailViewController.h"
#import <objc/runtime.h>

#pragma mark - 私有属性
@interface DYYYFLEXObjectExplorerViewController () <UIGestureRecognizerDelegate>
@property (nonatomic, readonly) DYYYFLEXSingleRowSection *descriptionSection;
@property (nonatomic, readonly) NSArray<DYYYFLEXTableViewSection *> *customSections;
...
@implementation DYYYFLEXObjectExplorerViewController

#pragma mark - 初始化

+ (instancetype)exploringObject:(id)target {
    return [self exploringObject:target customSection:[DYYYFLEXShortcutsSection forObject:target]];
}

+ (instancetype)exploringObject:(id)target customSection:(DYYYFLEXTableViewSection *)section {
...
+ (instancetype)exploringObject:(id)target customSections:(NSArray *)customSections {
    return [[self alloc]
        ...
        explorer:[DYYYFLEXObjectExplorer forObject:target]
        customSections:customSections
    ];
}

- (id)initWithObject:(id)target
            explorer:(__kindof DYYYFLEXObjectExplorer *)explorer
       customSections:(NSArray<DYYYFLEXTableViewSection *> *)customSections {
...
- (void)viewDidLoad {
    ...
    // 以避免被观察对象的KVO前缀
    self.title = [DYYYFLEXRuntimeUtility safeClassNameForObject:self.object];
    ...
    [self addToolbarItems:@[[UIBarButtonItem
        flex_itemWithImage:DYYYFLEXResources.moreIcon target:self action:@selector(moreButtonPressed:)
    ]]];
...
- (NSArray<DYYYFLEXTableViewSection *> *)nonemptySections {
    ...
    return [super.nonemptySections flex_filtered:^BOOL(DYYYFLEXTableViewSection *section, NSUInteger idx) {
        return section != self.descriptionSection;
    }];
}

- (NSArray<DYYYFLEXTableViewSection *> *)makeSections {
    DYYYFLEXObjectExplorer *explorer = self.explorer;
    
    // 描述部分仅适用于实例
    if (self.explorer.objectIsInstance) {
        _descriptionSection = [DYYYFLEXSingleRowSection
            title:@"描述" reuse:kFLEXMultilineCell cell:^(DYYYFLEXTableViewCell *cell) {
    ...
    // 对象图部分
    DYYYFLEXSingleRowSection *referencesSection = [DYYYFLEXSingleRowSection
        title:@"对象图" reuse:kFLEXDefaultCell cell:^(DYYYFLEXTableViewCell *cell) {
            cell.titleLabel.text = @"查看引用此对象的对象";
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        }
    ];
    referencesSection.selectionAction = ^(UIViewController *host) {
        UIViewController *references = [DYYYFLEXObjectListViewController
    ...
    NSMutableArray *sections = [NSMutableArray arrayWithArray:@[
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindProperties],
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindClassProperties],
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindIvars],
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindMethods],
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindClassMethods],
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindClassHierarchy],
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindProtocols],
        [DYYYFLEXMetadataSection explorer:self.explorer kind:FLEXMetadataKindOther],
        referencesSection
    ]];
...
- (void)shareButtonPressed:(UIBarButtonItem *)sender {
    [DYYYFLEXAlert makeSheet:^(DYYYFLEXAlert *make) {
        make.button(@"添加到书签").handler(^(NSArray<NSString *> *strings) {
            [DYYYFLEXBookmarkManager addBookmark:self.object];
        });
        ...
        make.button(@"复制地址").handler(^(NSArray<NSString *> *strings) {
            UIPasteboard.generalPasteboard.string = [DYYYFLEXUtility addressOfObject:self.object];
        });
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 其他行使用自动大小。
    DYYYFLEXTableViewSection *section = self.filterDelegate.sections[indexPath.section];
