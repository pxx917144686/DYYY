...
@interface DYYYFHSSnapshotView ()
@property (nonatomic, readonly) SCNView *sceneView;
@property (nonatomic) NSString *currentSummary;

/// Maps nodes by snapshot IDs
@property (nonatomic) NSDictionary<NSString *, DYYYFHSSnapshotNodes *> *nodesMap;
@property (nonatomic) NSInteger maxDepth;

@property (nonatomic) DYYYFHSSnapshotNodes *highlightedNodes;
...
@implementation DYYYFHSSnapshotView

#pragma mark - Initialization

+ (instancetype)delegate:(id<FHSSnapshotViewDelegate>)delegate {
    DYYYFHSSnapshotView *view = [self new];
...
- (void)initDepthSlider {
    _depthSlider = [DYYYFHSRangeSlider new];
    [self.depthSlider
        addTarget:self
...
- (void)setSelectedView:(DYYYFHSViewSnapshot *)view {
    // Ivar set in selectSnapshot:
    [self selectSnapshot:view ? self.nodesMap[view.view.identifier] : nil];
}

- (void)setSnapshots:(NSArray<DYYYFHSViewSnapshot *> *)snapshots {
    _snapshots = snapshots;

    // Create new scene (possibly discarding old scene)
    SCNScene *scene = [SCNScene new];
    scene.background.contents = DYYYFLEXColor.primaryBackgroundColor;
...
- (void)setHeaderExclusions:(NSArray<Class> *)headerExclusions {
    ...
    if (headerExclusions.count) {
        for (DYYYFHSSnapshotNodes *nodes in self.nodesMap.allValues) {
...
- (void)emphasizeViews:(NSArray<UIView *> *)emphasizedViews inSnapshots:(NSArray<DYYYFHSViewSnapshot *> *)snapshots {
    for (DYYYFHSViewSnapshot *snapshot in snapshots) {
        DYYYFHSSnapshotNodes *nodes = self.nodesMap[snapshot.view.identifier];
...
- (void)hideView:(DYYYFHSViewSnapshot *)view {
    ...
    DYYYFHSSnapshotNodes *nodes = self.nodesMap[view.view.identifier];
...
- (DYYYFHSSnapshotNodes *)nodesAtPoint:(CGPoint)point {
...
- (void)selectSnapshot:(DYYYFHSSnapshotNodes *)selected {
...
- (void)depthSliderDidChange:(DYYYFHSRangeSlider *)slider {
