...
#import "DYYYFLEXRuntimeExporter.h"
...
@interface DYYYFLEXRuntimeExporter ()
@property (nonatomic, readonly) DYYYFLEXSQLiteDatabaseManager *db;
...
@property (nonatomic, copy) NSArray<DYYYFLEXProtocol *> *protocols;
...
@implementation DYYYFLEXRuntimeExporter
...
+ (void)createRuntimeDatabaseAtPath:(NSString *)path
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        // 尝试创建并填充数据库，如果失败则中止
        DYYYFLEXRuntimeExporter *exporter = [self new];
...
- (BOOL)createAndPopulateDatabaseAtPath:(NSString *)path
    ...
    _db = [DYYYFLEXSQLiteDatabaseManager managerForDatabase:path];
...
- (void)loadMetadata:(void(^)(NSString *status))progress {
    ...
    if (self.loadedBundlePaths) {
        ...
        // 类
        self.classes = [[runtime classesForToken:DYYYFLEXSearchToken.any
    ...
    self.protocols = [[runtime copyProtocolList] flex_mapped:^id(Protocol *proto, NSUInteger idx) {
        return [DYYYFLEXProtocol protocol:proto];
    }];
}
...
- (BOOL)addImages:(void(^)(NSString *status))progress {
    ...
    DYYYFLEXSQLiteDatabaseManager *database = self.db;
...
NS_INLINE BOOL FREInsertProtocolMember(DYYYFLEXSQLiteDatabaseManager *db,
...
- (BOOL)addProtocols:(void(^)(NSString *status))progress {
    ...
    for (DYYYFLEXProtocol *proto in self.protocols) {
        ...
        // 必需的方法
        for (DYYYFLEXMethodDescription *method in proto.requiredMethods) {
        ...
        // 可选的方法
        for (DYYYFLEXMethodDescription *method in proto.optionalMethods) {
            NSString *selector = NSStringFromSelector(method.selector);
        ...
        if (@available(iOS 10, *)) {
            // 必需的属性
            for (DYYYFLEXProperty *property in proto.requiredProperties) {
            ...
            // 可选的属性
            for (DYYYFLEXProperty *property in proto.optionalProperties) {
                BOOL success = FREInsertProtocolMember(
            ...
            // 仅...属性
            for (DYYYFLEXProperty *property in proto.properties) {
                BOOL success = FREInsertProtocolMember(
...
- (BOOL)addProtocolConformances:(void(^)(NSString *status))progress {
    ...
    for (DYYYFLEXProtocol *proto in self.protocols) {
        ...
        for (DYYYFLEXProtocol *conform in proto.protocols) {
...
- (BOOL)addClassConformances:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        ...
        for (DYYYFLEXProtocol *conform in FLEXGetConformedProtocols(cls)) {
...
- (BOOL)addIvars:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        for (DYYYFLEXIvar *ivar in FLEXGetAllIvars(cls)) {
            // 插入类型编码
...
- (BOOL)addMethods:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        ...
        // 用于处理每个消息的块
        BOOL (^insert)(DYYYFLEXMethod *, NSNumber *) = ^BOOL(DYYYFLEXMethod *method, NSNumber *instance) {
            // 首先插入选择器和签名
        ...
        for (DYYYFLEXMethod *method in FLEXGetAllMethods(cls, YES)) {
            if (!insert(method, @YES)) {
                return NO;
            }
        }
        for (DYYYFLEXMethod *method in FLEXGetAllMethods(object_getClass(cls), NO)) {
            if (!insert(method, @NO)) {
                return NO;
            }
        }
...
- (BOOL)addProperties:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        ...
        // 用于处理每个消息的块
        BOOL (^insert)(DYYYFLEXProperty *, NSNumber *) = ^BOOL(DYYYFLEXProperty *property, NSNumber *instance) {
            DYYYFLEXPropertyAttributes *attrs = property.attributes;
            ...
            // 首先插入类型编码
            NSInteger size = [DYYYFLEXTypeEncodingParser
        ...
        for (DYYYFLEXProperty *property in FLEXGetAllProperties(cls)) {
            if (!insert(property, @YES)) {
                return NO;
            }
        }
        for (DYYYFLEXProperty *property in FLEXGetAllProperties(object_getClass(cls))) {
...
- (BOOL)addMethodSignature:(DYYYFLEXMethod *)method {
...
- (BOOL)executeInsert:(NSString *)statement
    ...
    // 插入
    DYYYFLEXSQLiteDatabaseManager *database = _db;
