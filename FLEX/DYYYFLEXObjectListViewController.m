//
//  DYYYFLEXObjectListViewController.m
//  Flipboard
//
...
#import "DYYYFLEXHeapEnumerator.h"
#import "DYYYFLEXObjectRef.h"
...
@interface DYYYFLEXObjectListViewController ()
...
@property (nonatomic, copy) NSArray<DYYYFLEXMutableListSection *> *sections;
@property (nonatomic, copy) NSArray<DYYYFLEXMutableListSection *> *allSections;

@property (nonatomic, readonly, nullable) NSArray<DYYYFLEXObjectRef *> *references;
@property (nonatomic, readonly) NSArray<NSPredicate *> *predicates;
@property (nonatomic, readonly) NSArray<NSString *> *sectionTitles;

@end

@implementation DYYYFLEXObjectListViewController
...
+ (NSPredicate *)defaultPredicateForSection:(NSInteger)section {
    ...
    BOOL(^isKVORelated)(DYYYFLEXObjectRef *, NSDictionary *) = ^BOOL(DYYYFLEXObjectRef *ref, NSDictionary *bindings) {
    ...
    BOOL(^isConstraintRelated)(DYYYFLEXObjectRef *, NSDictionary *) = ^BOOL(DYYYFLEXObjectRef *ref, NSDictionary *bindings) {
    ...
    BOOL(^isFLEXClass)(DYYYFLEXObjectRef *, NSDictionary *) = ^BOOL(DYYYFLEXObjectRef *ref, NSDictionary *bindings) {
        return [ref.reference hasPrefix:@"FLEX"];
    };

    BOOL(^isEssential)(DYYYFLEXObjectRef *, NSDictionary *) = ^BOOL(DYYYFLEXObjectRef *ref, NSDictionary *bindings) {
        return !(
...
- (id)initWithReferences:(nullable NSArray<DYYYFLEXObjectRef *> *)references {
    return [self initWithReferences:references predicates:nil sectionTitles:nil];
}

- (id)initWithReferences:(NSArray<DYYYFLEXObjectRef *> *)references
...
+ (UIViewController *)instancesOfClassWithName:(NSString *)className retained:(BOOL)retain {
    NSArray<DYYYFLEXObjectRef *> *references = [DYYYFLEXHeapEnumerator
    ...
    DYYYFLEXObjectListViewController *controller = [[self alloc] initWithReferences:references];
...
+ (instancetype)subclassesOfClassWithName:(NSString *)className {
    NSArray<DYYYFLEXObjectRef *> *references = [DYYYFLEXRuntimeUtility subclassesOfClassWithName:className];
    DYYYFLEXObjectListViewController *controller = [[self alloc] initWithReferences:references];
...
+ (instancetype)objectsWithReferencesToObject:(id)object retained:(BOOL)retain {
    NSArray<DYYYFLEXObjectRef *> *instances = [DYYYFLEXHeapEnumerator
        objectsWithReferencesToObject:object retained:retain
    ];

    DYYYFLEXObjectListViewController *viewController = [[self alloc]
    ...
    viewController.title = [NSString stringWithFormat:@"引用 %@ %p",
        [DYYYFLEXRuntimeUtility safeClassNameForObject:object], object
    ];
...
- (NSArray<DYYYFLEXMutableListSection *> *)makeSections {
...
- (DYYYFLEXMutableListSection *)makeSection:(NSArray *)rows title:(NSString *)title {
    DYYYFLEXMutableListSection *section = [DYYYFLEXMutableListSection list:rows
        cellConfiguration:^(DYYYFLEXTableViewCell *cell, DYYYFLEXObjectRef *ref, NSInteger row) {
            cell.textLabel.text = ref.reference;
            cell.detailTextLabel.text = ref.summary;
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        } filterMatcher:^BOOL(NSString *filterText, DYYYFLEXObjectRef *ref) {
    ...
    section.selectionHandler = ^(UIViewController *host, DYYYFLEXObjectRef *ref) {
        [host.navigationController pushViewController:[
            DYYYFLEXObjectExplorerFactory explorerViewControllerForObject:ref.object
        ] animated:YES];
    };
