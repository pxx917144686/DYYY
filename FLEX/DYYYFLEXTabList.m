...
@interface DYYYFLEXTabList () {
    NSMutableArray *_openTabs;
    NSMutableArray *_openTabSnapshots;
}
@end
#pragma mark -
@implementation DYYYFLEXTabList

#pragma mark 初始化

+ (DYYYFLEXTabList *)sharedList {
    static DYYYFLEXTabList *sharedList = nil;
...
- (void)addTab:(UINavigationController *)newTab {
    ...
    [_openTabSnapshots addObject:[DYYYFLEXUtility previewImageForView:newTab.view]];
    _activeTab = newTab;
...
- (void)updateSnapshotForActiveTab {
    if (self.activeTabIndex != NSNotFound) {
        UIImage *newSnapshot = [DYYYFLEXUtility previewImageForView:self.activeTab.view];
