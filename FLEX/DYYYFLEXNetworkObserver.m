...
@interface DYYYFLEXInternalRequestState : NSObject
...
@implementation DYYYFLEXInternalRequestState

@end

@interface DYYYFLEXNetworkObserver (NSURLConnectionHelpers)
...
@interface DYYYFLEXNetworkObserver (NSURLSessionTaskHelpers)
...
@interface DYYYFLEXNetworkObserver ()

@property (nonatomic) NSMutableDictionary<NSString *, DYYYFLEXInternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic) dispatch_queue_t queue;

@end

@implementation DYYYFLEXNetworkObserver
...
+ (instancetype)sharedObserver {
    static DYYYFLEXNetworkObserver *sharedObserver = nil;
...
static void _logos_method$_ungrouped$FIRDocumentReference$getDocumentWithCompletion$(
    ...
    // 生成事务ID
    NSString *requestID = [DYYYFLEXNetworkObserver nextRequestID];
    
    // 记录事务开始
    [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRDocumentWillFetch:self withTransactionID:requestID];
    // 钩住回调
    FIRDocumentSnapshotBlock orig = completion;
    completion = ^(FIRDocumentSnapshot *document, NSError *error) {
        [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRDocumentDidFetch:document error:error transactionID:requestID];
        if (orig != nil) {
            orig(document, error);
        }
    };
...
static void _logos_method$_ungrouped$FIRQuery$getDocumentsWithCompletion$(
    ...
    // 记录事务开始
    [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRQueryWillFetch:self withTransactionID:requestID];
    // 钩住回调
    FIRQuerySnapshotBlock orig = completion;
    completion = ^(FIRQuerySnapshot *query, NSError *error) {
        [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRQueryDidFetch:query error:error transactionID:requestID];
        if (orig != nil) {
            orig(query, error);
        }
    };
...
static void _logos_method$_ungrouped$FIRDocumentReference$setData$merge$completion$(
    ...
    // 记录事务开始
    [DYYYFLEXNetworkRecorder.defaultRecorder
    ...
    completion = ^(NSError *error) {
        [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRDidSetData:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
...
static void _logos_method$_ungrouped$FIRDocumentReference$updateData$completion$(
    ...
    // 记录事务开始
    [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRWillUpdateData:self fields:fields transactionID:requestID];
    // 钩住回调
    void (^orig)(NSError *) = completion;
    completion = ^(NSError *error) {
        [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRDidUpdateData:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
...
static void _logos_method$_ungrouped$FIRDocumentReference$deleteDocumentWithCompletion$(
    ...
    // 记录事务开始
    [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRWillDeleteDocument:self transactionID:requestID];
    // 钩住回调
    void (^orig)(NSError *) = completion;
    completion = ^(NSError *error) {
        [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRDidDeleteDocument:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
...
static FIRDocumentReference * _logos_method$_ungrouped$FIRCollectionReference$addDocumentWithData$completion$(
    ...
    void (^orig)(NSError *) = completion;
    completion = ^(NSError *error) {
        [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRDidAddDocument:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
    ...
    // 记录事务开始
    [DYYYFLEXNetworkRecorder.defaultRecorder recordFIRWillAddDocument:self document:ret transactionID:requestID];
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [DYYYFLEXNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [DYYYFLEXUtility swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [DYYYFLEXNetworkObserver.sharedObserver connectionWillCancel:slf];
    ...
    IMP implementation = imp_implementationWithBlock(^(NSURLSessionTask *slf) {
        ...
        if (@available(iOS 11.0, *)) {
            ...
            if (![slf isKindOfClass:[AVAggregateAssetDownloadTask class]]) {
                ...
                [DYYYFLEXNetworkObserver.sharedObserver URLSessionTaskWillResume:slf];
            }
        }
    ...
    dispatch_once(&onceToken, ^{
        ...
        SendAsyncRequestBlock swizzleBlock = ^(Class slf,
            ...
            if (DYYYFLEXNetworkObserver.isEnabled) {
                ...
                [DYYYFLEXNetworkRecorder.defaultRecorder recordMechanism:mechanism forRequestID:requestID];
        ...
        [DYYYFLEXUtility replaceImplementationOfKnownSelector:selector
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods:(Class)sessionClass {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([DYYYFLEXUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionNewTaskMethod swizzleBlock = ^NSURLSessionTask *(NSURLSession *slf,
                ...
                // 检查网络观察是否开启以及是否提供了回调
                if (DYYYFLEXNetworkObserver.isEnabled && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
                    // "钩住"完成块
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls {
    ...
    WillSendRequestBlock undefinedBlock = ^NSURLRequest *(id slf,
                                                          ...
                                                          NSURLRequest *request,
                                                          NSURLResponse *response) {
        [DYYYFLEXNetworkObserver.sharedObserver
    ...
    [DYYYFLEXUtility replaceImplementationOfSelector:selector
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls {
    ...
    DidReceiveResponseBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf,
                                               NSURLConnection *connection,
                                               NSURLResponse *response) {
        [DYYYFLEXNetworkObserver.sharedObserver connection:connection
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls {
    ...
    FinishLoadingBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [DYYYFLEXNetworkObserver.sharedObserver connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls {
    ...
    DidReceiveDataBlock undefinedBlock = ^(id<NSURLSessionDataDelegate> slf,
                                           ...
                                           NSURLSessionDataTask *dataTask,
                                           NSData *data) {
        [DYYYFLEXNetworkObserver.sharedObserver URLSession:session
...
- (id)init {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create(
            "com.flex.DYYYFLEXNetworkObserver", DISPATCH_QUEUE_SERIAL
        );
    }
...
- (DYYYFLEXInternalRequestState *)requestStateForRequestID:(NSString *)requestID {
    DYYYFLEXInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [DYYYFLEXInternalRequestState new];
...
@implementation DYYYFLEXNetworkObserver (NSURLConnectionHelpers)
...
- (void)connection:(NSURLConnection *)connection
  ...
  redirectResponse:(NSURLResponse *)response
    ...
    [self performBlock:^{
        ...
        DYYYFLEXInternalRequestState *requestState = [self requestStateForRequestID:requestID];
...
@implementation DYYYFLEXNetworkObserver (NSURLSessionTaskHelpers)
...
- (void)websocketTask:(NSURLSessionWebSocketTask *)task
    ...
    [self performBlock:^{
//        NSString *requestID = [[self class] requestIDForConnectionOrTask:task];
        [DYYYFLEXNetworkRecorder.defaultRecorder recordWebsocketMessageSend:message task:task];
    }];
}
