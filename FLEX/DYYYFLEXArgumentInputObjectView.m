...
@interface DYYYFLEXArgumentInputObjectView ()
...
@implementation DYYYFLEXArgumentInputObjectView
...
- (id)inputValue {
    switch (self.inputType) {
        case FLEXArgInputObjectTypeJSON:
            return [DYYYFLEXRuntimeUtility objectValueFromEditableJSONString:self.inputTextView.text];
        case FLEXArgInputObjectTypeAddress: {
...
- (void)populateTextAreaFromValue:(id)value {
    if (!value) {
        ...
        if (self.inputType == FLEXArgInputObjectTypeJSON) {
            self.inputTextView.text = [DYYYFLEXRuntimeUtility editableJSONStringForObject:value];
...
+ (FLEXArgInputObjectType)preferredDefaultTypeForObjCType:(const char *)type withCurrentValue:(id)value {
    ...
    if (value) {
        ...
        if ([DYYYFLEXRuntimeUtility editableJSONStringForObject:value]) {
