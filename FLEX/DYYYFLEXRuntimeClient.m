...
@interface DYYYFLEXRuntimeClient () {
...
@implementation DYYYFLEXRuntimeClient

#pragma mark - 初始化

+ (instancetype)runtime {
    static DYYYFLEXRuntimeClient *runtime;
...
- (NSMutableArray<NSString *> *)bundlePathsForToken:(DYYYFLEXSearchToken *)token {
    if (self.imagePaths.count) {
        TBWildcardOptions options = token.options;
        NSString *query = token.string;
...
- (NSMutableArray<NSString *> *)classesForToken:(DYYYFLEXSearchToken *)token inBundles:(NSMutableArray<NSString *> *)bundles {
...
- (NSMutableArray<NSString *> *)_classesForToken:(DYYYFLEXSearchToken *)token inBundles:(NSMutableArray<NSString *> *)bundles {
    TBWildcardOptions options = token.options;
    NSString *query = token.string;
...
- (NSArray<NSMutableArray<DYYYFLEXMethod *> *> *)methodsForToken:(DYYYFLEXSearchToken *)token
        ...
        switch (options) {
            ...
            case TBWildcardOptionsNone: {
                ...
                return @[[classes flex_mapped:^id(NSString *name, NSUInteger idx) {
                    ...
                    // 方法是绝对的
                    return [DYYYFLEXMethod selector:sel class:cls];
                }]];
            }
            ...
            default: {
                ...
                if (options & TBWildcardOptionsPrefix &&
                    ...
                    return [classes flex_mapped:^NSArray *(NSString *name, NSUInteger idx) {
                        Class cls = NSClassFromString(name);
                        return [[cls flex_allMethods] flex_mapped:^id(DYYYFLEXMethod *method, NSUInteger idx) {
                ...
                // 仅当"方法以选择器开头"时
                    ...
                    return [classes flex_mapped:^NSArray *(NSString *name, NSUInteger idx) {
                        ...
                        id mapping = ^id(DYYYFLEXMethod *method) {
                            // 方法是后缀通配符
