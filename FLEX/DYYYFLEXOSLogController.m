...
@interface DYYYFLEXOSLogController ()

+ (DYYYFLEXOSLogController *)sharedLogController;

@property (nonatomic) void (^updateHandler)(NSArray<DYYYFLEXSystemLogMessage *> *);
...
@implementation DYYYFLEXOSLogController
...
+ (instancetype)sharedLogController {
    static DYYYFLEXOSLogController *shared = nil;
...
+ (instancetype)withUpdateHandler:(void(^)(NSArray<DYYYFLEXSystemLogMessage *> *newMessages))newMessagesHandler {
    DYYYFLEXOSLogController *shared = [self sharedLogController];
...
- (BOOL)handleStreamEntry:(os_activity_stream_entry_t)entry error:(int)error {
    ...
    if (!error && entry) {
        if (entry->type == OS_ACTIVITY_STREAM_TYPE_LOG_MESSAGE ||
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                DYYYFLEXSystemLogMessage *message = [DYYYFLEXSystemLogMessage logMessageFromDate:date text:msg];
                if (self.persistent) {
