...
@interface DYYYFLEXViewShortcuts ()
@property (nonatomic, readonly) UIView *view;
@end

@implementation DYYYFLEXViewShortcuts
...
+ (instancetype)forObject:(UIView *)view {
    ...
    // 是否为 nil 或已更改。
    UIViewController *controller = [DYYYFLEXViewShortcuts nearestViewControllerForView:view];

    return [self forObject:view additionalRows:@[
        [DYYYFLEXActionShortcut title:@"最近的视图控制器"
            subtitle:^NSString *(id view) {
                return [DYYYFLEXRuntimeUtility safeDescriptionForObject:controller];
            }
            viewer:^UIViewController *(id view) {
                return [DYYYFLEXObjectExplorerFactory explorerViewControllerForObject:controller];
            }
            accessoryType:^UITableViewCellAccessoryType(id view) {
                return controller ? UITableViewCellAccessoryDisclosureIndicator : UITableViewCellAccessoryNone;
            }
        ],
        [DYYYFLEXActionShortcut title:@"预览图像" subtitle:^NSString *(UIView *view) {
                return !CGRectIsEmpty(view.bounds) ? @"" : @"空边界时不可用";
            }
            viewer:^UIViewController *(UIView *view) {
                return [DYYYFLEXImagePreviewViewController previewForView:view];
            }
