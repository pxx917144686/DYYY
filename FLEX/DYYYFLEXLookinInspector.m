...
@implementation DYYYFLEXLookinViewNode
...
@interface DYYYFLEXLookinInspector ()
@property (nonatomic, strong) UIWindow *inspectorWindow;
@property (nonatomic, strong) UIView *overlayView;
@property (nonatomic, strong) NSMutableArray<DYYYFLEXLookinViewNode *> *mutableViewHierarchy;
...
@implementation DYYYFLEXLookinInspector

+ (instancetype)sharedInstance {
    static DYYYFLEXLookinInspector *instance = nil;
...
- (NSArray<DYYYFLEXLookinViewNode *> *)viewHierarchy {
...
- (void)refreshViewHierarchy {
    ...
    UIView *keyWindow = [UIApplication sharedApplication].keyWindow;
    if (keyWindow) {
        DYYYFLEXLookinViewNode *rootNode = [self createNodeForView:keyWindow depth:0];
...
- (DYYYFLEXLookinViewNode *)createNodeForView:(UIView *)view depth:(NSInteger)depth {
    DYYYFLEXLookinViewNode *node = [[DYYYFLEXLookinViewNode alloc] init];
    node.view = view;
...
- (void)buildHierarchyForNode:(DYYYFLEXLookinViewNode *)node {
    ...
    for (UIView *subview in node.view.subviews) {
        ...
        DYYYFLEXLookinViewNode *childNode = [self createNodeForView:subview depth:node.depth + 1];
...
- (DYYYFLEXLookinViewNode *)nodeForView:(UIView *)view {
    return [self findNodeForView:view inNodes:self.viewHierarchy];
}

- (DYYYFLEXLookinViewNode *)findNodeForView:(UIView *)view inNodes:(NSArray<DYYYFLEXLookinViewNode *> *)nodes {
    for (DYYYFLEXLookinViewNode *node in nodes) {
        if (node.view == view) {
            return node;
        }
        
        DYYYFLEXLookinViewNode *foundNode = [self findNodeForView:view inNodes:node.children];
        if (foundNode) {
            return foundNode;
        }
    }
...
- (NSArray<DYYYFLEXLookinViewNode *> *)flattenedHierarchy {
    NSMutableArray *flattened = [NSMutableArray new];
    [self flattenNodes:self.viewHierarchy intoArray:flattened];
    return [flattened copy];
}

- (void)flattenNodes:(NSArray<DYYYFLEXLookinViewNode *> *)nodes intoArray:(NSMutableArray *)array {
    for (DYYYFLEXLookinViewNode *node in nodes) {
...
- (NSArray *)serializeHierarchy:(NSArray<DYYYFLEXLookinViewNode *> *)nodes {
...
- (void)render3DHierarchy {
    ...
    // 为每个视图创建3D表示
    for (DYYYFLEXLookinViewNode *node in self.viewHierarchy) {
        [self create3DRepresentationForNode:node];
    }
}

- (UIView *)create3DRepresentationForNode:(DYYYFLEXLookinViewNode *)node {
    ...
    // 处理子视图
    for (DYYYFLEXLookinViewNode *childNode in node.children) {
...
- (UIView *)findOriginalViewFor3DView:(UIView *)view3D {
    ...
    // 简化实现，通过frame匹配
    for (DYYYFLEXLookinViewNode *node in [self flattenHierarchy:self.viewHierarchy]) {
...
- (NSArray<DYYYFLEXLookinViewNode *> *)flattenHierarchy:(NSArray<DYYYFLEXLookinViewNode *> *)hierarchy {
    NSMutableArray *flattened = [NSMutableArray new];
    
    for (DYYYFLEXLookinViewNode *node in hierarchy) {
