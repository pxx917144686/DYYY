...
@implementation DYYYFLEXObjectRef
...
+ (NSArray<DYYYFLEXObjectRef *> *)referencingAll:(NSArray *)objects retained:(BOOL)retain {
    return [objects flex_mapped:^id(id obj, NSUInteger idx) {
        return [self referencing:obj showSummary:YES retained:retain];
    }];
}

+ (NSArray<DYYYFLEXObjectRef *> *)referencingClasses:(NSArray<Class> *)classes {
...
- (id)initWithObject:(__unsafe_unretained id)object
    ...
    if (self) {
        ...
        NSString *class = [DYYYFLEXRuntimeUtility safeClassNameForObject:object];
        if (ivar) {
...
- (NSString *)summary {
    if (self.wantsSummary) {
        if (!_summary) {
            _summary = [DYYYFLEXRuntimeUtility summaryForObject:self.object];
        }
