//
//  DYYYFLEXDefaultsContentSection.m
//  FLEX
//
...
@interface DYYYFLEXDefaultsContentSection ()
@property (nonatomic) NSUserDefaults *defaults;
@property (nonatomic) NSArray *keys;
@property (nonatomic, readonly) NSDictionary *unexcludedDefaults;
@end

@implementation DYYYFLEXDefaultsContentSection
...
+ (instancetype)forDefaults:(NSUserDefaults *)userDefaults {
    DYYYFLEXDefaultsContentSection *section = [self forReusableFuture:^id(DYYYFLEXDefaultsContentSection *section) {
...
- (void (^)(__kindof UIViewController *))didPressInfoButtonAction:(NSInteger)row {
    return ^(UIViewController *host) {
        if ([DYYYFLEXDefaultEditorViewController canEditDefaultWithValue:[self objectForRow:row]]) {
            // 我们使用titleForRow:获取键，因为self.keys不一定与显示的键顺序相同
            DYYYFLEXVariableEditorViewController *controller = [DYYYFLEXDefaultEditorViewController
            ...
            [DYYYFLEXAlert showAlert:@"NO..." message:@"我们无法编辑此条目 :(" from:host];
        }
    };
}
