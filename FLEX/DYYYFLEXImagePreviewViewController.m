//
//  DYYYFLEXImagePreviewViewController.m
//  Flipboard
//
...
@interface DYYYFLEXImagePreviewViewController () <UIScrollViewDelegate>
...
#pragma mark -
@implementation DYYYFLEXImagePreviewViewController

#pragma mark 初始化

+ (instancetype)previewForView:(UIView *)view {
    return [self forImage:[DYYYFLEXUtility previewImageForView:view]];
}

+ (instancetype)previewForLayer:(CALayer *)layer {
    return [self forImage:[DYYYFLEXUtility previewImageForLayer:layer]];
}
...
- (id)initWithImage:(UIImage *)image {
    ...
    if (self) {
        self.title = @"预览";
        self.image = image;
        _backgroundColors = @[DYYYFLEXResources.checkerPatternColor, UIColor.whiteColor, UIColor.blackColor];
    }
...
- (void)actionButtonPressed:(id)sender {
    ...
    UIViewController *activityVC = [DYYYFLEXActivityViewController sharing:@[self.image] source:sender];
