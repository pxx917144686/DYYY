...
BOOL FLEXGetSizeAndAlignment(const char *type, NSUInteger *sizep, NSUInteger *alignp) {
    NSInteger size = 0;
    ssize_t align = 0;
    size = [DYYYFLEXTypeEncodingParser sizeForTypeEncoding:@(type) alignment:&align];
...
@interface DYYYFLEXTypeEncodingParser ()
...
@implementation DYYYFLEXTypeEncodingParser
...
+ (BOOL)methodTypeEncodingSupported:(NSString *)typeEncoding cleaned:(NSString * __autoreleasing *)cleanedEncoding {
    ...
    DYYYFLEXTypeEncodingParser *parser = [[self alloc] initWithObjCTypes:typeEncoding];
...
+ (FLEXTypeInfo)parseType:(NSString *)type cleaned:(NSString * __autoreleasing *)cleanedEncoding {
    DYYYFLEXTypeEncodingParser *parser = [[self alloc] initWithObjCTypes:type];
    FLEXTypeInfo info = [parser parseNextType];
    if (cleanedEncoding) {
