//
//  DYYYFLEXNetworkSettingsController.m
//  FLEXInjected
//
...
@interface DYYYFLEXNetworkSettingsController () <UIActionSheetDelegate>
...
@implementation DYYYFLEXNetworkSettingsController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self disableToolbar];
    self.hostDenylist = DYYYFLEXNetworkRecorder.defaultRecorder.hostDenylist.mutableCopy;
    ...
    self.observerSwitch.on = DYYYFLEXNetworkObserver.enabled;
    [self.observerSwitch addTarget:self
        action:@selector(networkDebuggingToggled:)
        forControlEvents:UIControlEventValueChanged
    ];
    
    self.cacheMediaSwitch.on = DYYYFLEXNetworkRecorder.defaultRecorder.shouldCacheMediaResponses;
    ...
    self.cacheLimitValue = DYYYFLEXNetworkRecorder.defaultRecorder.responseCacheByteLimit;
...
- (void)setCacheLimitValue:(float)cacheLimitValue {
    ...
    [DYYYFLEXNetworkRecorder.defaultRecorder setResponseCacheByteLimit:cacheLimitValue];
}
...
- (void)networkDebuggingToggled:(UISwitch *)sender {
    DYYYFLEXNetworkObserver.enabled = sender.isOn;
}

- (void)cacheMediaResponsesToggled:(UISwitch *)sender {
    DYYYFLEXNetworkRecorder.defaultRecorder.shouldCacheMediaResponses = sender.isOn;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [DYYYFLEXAlert makeAlert:^(DYYYFLEXAlert *make) {
        ...
        make.button(@"重置").destructiveStyle().handler(^(NSArray<NSString *> *strings) {
            ...
            [DYYYFLEXNetworkRecorder.defaultRecorder.hostDenylist removeAllObjects];
...
forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [DYYYFLEXNetworkRecorder.defaultRecorder.hostDenylist removeObject:host];
    [DYYYFLEXNetworkRecorder.defaultRecorder synchronizeDenylist];
