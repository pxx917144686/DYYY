...
+ (void)browseBundleAsDatabase:(NSBundle *)bundle host:(UIViewController *)host name:(NSString *)name {
    ...
    UIAlertController *progress = [DYYYFLEXAlert makeAlert:^(DYYYFLEXAlert *make) {
        make.title(@"正在生成数据库");
    ...
    [host presentViewController:progress animated:YES completion:^{
        ...
        // 生成数据库并显示进度
        [DYYYFLEXRuntimeExporter createRuntimeDatabaseAtPath:path
            ...
            progressHandler:^(NSString *status) {
                ...
                // 浏览数据库
                    ...
                    [host.navigationController pushViewController:[
                        [DYYYFLEXTableListViewController alloc] initWithPath:path
                    ] animated:YES];
                }
            }
        ];
