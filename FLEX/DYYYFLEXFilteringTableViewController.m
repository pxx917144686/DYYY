//
//  DYYYFLEXFilteringTableViewController.m
//  FLEX
//
...
@interface DYYYFLEXFilteringTableViewController ()

@end

@implementation DYYYFLEXFilteringTableViewController
...
- (void)_registerCellsForReuse {
    for (DYYYFLEXTableViewSection *section in self.filterDelegate.allSections) {
...
- (NSArray<DYYYFLEXTableViewSection *> *)nonemptySections {
    return [self.filterDelegate.allSections flex_filtered:^BOOL(DYYYFLEXTableViewSection *section, NSUInteger idx) {
...
- (void)setAllSections:(NSArray<DYYYFLEXTableViewSection *> *)allSections {
    _allSections = allSections.copy;
    // 只显示非空部分
    self.sections = self.nonemptySections;
}

- (void)setSections:(NSArray<DYYYFLEXTableViewSection *> *)sections {
    // 允许部分随时重新加载表视图的一部分
    [sections enumerateObjectsUsingBlock:^(DYYYFLEXTableViewSection *s, NSUInteger idx, BOOL *stop) {
