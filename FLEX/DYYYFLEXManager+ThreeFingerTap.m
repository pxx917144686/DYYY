#import "DYYYFLEXManager+ThreeFingerTap.h"
...
@implementation DYYYFLEXManager (ThreeFingerTap)
    ...
    if (!flex_threeFingerLongPressGesture) {
        flex_threeFingerLongPressGesture = [UILongPressGestureRecognizer flex_action:^(UIGestureRecognizer *gesture) {
            if (gesture.state == UIGestureRecognizerStateBegan) {
                if ([DYYYFLEXManager sharedManager]) {
                    [[DYYYFLEXManager sharedManager] toggleExplorer];
                }
            }
        }];
...
+ (UIWindow *)flex_findTargetWindow {
        ...
        for (UIScene *scene in [UIApplication sharedApplication].connectedScenes) {
            if (scene.activationState == UISceneActivationStateForegroundActive && [scene isKindOfClass:[UIWindowScene class]]) {
                ...
                for (UIWindow *window in windowScene.windows) {
                    ...
                    if (window.isKeyWindow && ![NSStringFromClass(window.class) isEqualToString:@"DYYYFLEXWindow"]) {
                ...
                // 备选：活动场景中的第一个非FLEXWindow
                 if (!applicationWindow) {
                    for (UIWindow *window in windowScene.windows) {
                        if (![NSStringFromClass(window.class) isEqualToString:@"DYYYFLEXWindow"]) {
                            applicationWindow = window;
                            break;
                        }
                    }
    ...
    if (!applicationWindow) {
        ...
        if (applicationWindow && [NSStringFromClass(applicationWindow.class) isEqualToString:@"DYYYFLEXWindow"]) {
            UIWindow* fallbackWindow = nil;
            for (UIWindow *window in windows) {
                if (![NSStringFromClass(window.class) isEqualToString:@"DYYYFLEXWindow"] && !window.isHidden) {
            ...
            if (![NSStringFromClass(applicationWindow.class) isEqualToString:@"DYYYFLEXWindow"] || !fallbackWindow) {
