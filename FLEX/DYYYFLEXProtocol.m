...
@implementation DYYYFLEXProtocol
...
- (void)examine {
    ...
    _protocols = [NSArray flex_forEachUpTo:pccount map:^id(NSUInteger i) {
        return [DYYYFLEXProtocol protocol:protocols[i]];
    }];
    free(protocols);
    ...
    NSArray *rMethods = [NSArray flex_forEachUpTo:mdrcount map:^id(NSUInteger i) {
        return [DYYYFLEXMethodDescription description:objcrMethods[i] instance:YES];
    }];
    free(objcrMethods);
    ...
    _requiredMethods = [[NSArray flex_forEachUpTo:mdrcount map:^id(NSUInteger i) {
        return [DYYYFLEXMethodDescription description:objcrMethods[i] instance:NO];
    ...
    NSArray *oMethods = [NSArray flex_forEachUpTo:mdocount map:^id(NSUInteger i) {
        return [DYYYFLEXMethodDescription description:objcoMethods[i] instance:YES];
    }];
    free(objcoMethods);
    ...
    _optionalMethods = [[NSArray flex_forEachUpTo:mdocount map:^id(NSUInteger i) {
        return [DYYYFLEXMethodDescription description:objcoMethods[i] instance:NO];
    ...
    if (@available(iOS 10.0, *)) {
        ...
        NSArray *rProperties = [NSArray flex_forEachUpTo:prrcount map:^id(NSUInteger i) {
            return [DYYYFLEXProperty property:rProps[i] onClass:instance];
        }];
        free(rProps);
        ...
        NSArray *oProperties = [NSArray flex_forEachUpTo:prrcount map:^id(NSUInteger i) {
            return [DYYYFLEXProperty property:oProps[i] onClass:meta];
        }];
        free(oProps);
        ...
        _properties = [NSArray flex_forEachUpTo:prcount map:^id(NSUInteger i) {
            return [DYYYFLEXProperty property:objcproperties[i]];
        }];
...
#pragma mark DYYYFLEXMethodDescription

@implementation DYYYFLEXMethodDescription
