...
@implementation DYYYFLEXDoKitLeakInfo
...
@interface DYYYFLEXDoKitMemoryLeakDetector ()
@property (nonatomic, strong) NSMutableArray<DYYYFLEXDoKitLeakInfo *> *mutableLeakInfos;
...
@implementation DYYYFLEXDoKitMemoryLeakDetector

+ (instancetype)sharedInstance {
    static DYYYFLEXDoKitMemoryLeakDetector *instance = nil;
...
- (NSMutableArray<DYYYFLEXDoKitLeakInfo *> *)leakInfos {
...
- (void)detectLeakForClass:(NSString *)className currentCount:(NSUInteger)currentCount previousCount:(NSUInteger)previousCount {
    ...
    for (DYYYFLEXDoKitLeakInfo *info in self.mutableLeakInfos) {
    ...
    if (!alreadyReported) {
        // 创建新的泄漏信息
        DYYYFLEXDoKitLeakInfo *leakInfo = [[DYYYFLEXDoKitLeakInfo alloc] init];
...
- (void)flex_viewDidDisappear:(BOOL)animated {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (self) {
            ...
            [[[DYYYFLEXDoKitMemoryLeakDetector sharedInstance] mutableLeakInfos] addObject:leakInfo];
