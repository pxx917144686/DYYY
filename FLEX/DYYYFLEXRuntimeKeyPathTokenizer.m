//
//  DYYYFLEXRuntimeKeyPathTokenizer.m
//  FLEX
//
...
@implementation DYYYFLEXRuntimeKeyPathTokenizer
...
+ (DYYYFLEXRuntimeKeyPath *)tokenizeString:(NSString *)userInput {
    ...
    DYYYFLEXSearchToken *bundle    = [self scanToken:scanner allowed:filenameAllowed first:filenameAllowed];
    DYYYFLEXSearchToken *cls       = [self scanToken:scanner allowed:identifierAllowed first:firstAllowed];
    DYYYFLEXSearchToken *method    = tokens > 2 ? [self scanMethodToken:scanner instance:&instance] : nil;

    return [DYYYFLEXRuntimeKeyPath bundle:bundle
                       class:cls
                      method:method
                  isInstance:instance
                      string:userInput];
...
+ (DYYYFLEXSearchToken *)scanToken:(NSScanner *)scanner allowed:(NSCharacterSet *)allowedChars first:(NSCharacterSet *)first {
    if (scanner.isAtEnd) {
        if ([scanner.string hasSuffix:@"."] && ![scanner.string hasSuffix:@"\\."]) {
            return [DYYYFLEXSearchToken string:nil options:TBWildcardOptionsAny];
        }
        return nil;
    }
    ...
    if ([scanner scanString:@"*." intoString:nil]) {
        ...
        if (scanner.isAtEnd) {
            return DYYYFLEXSearchToken.any;
        }
    ...
    while (!stop && ![scanner scanString:@"." intoString:&tmp] && !scanner.isAtEnd) {
        ...
        // 扫描'\.'或尾随'\'
            ...
            if ([scanner scanString:@"." intoString:nil]) {
                ...
                // 如果在末尾，忽略不后跟句点的正斜杠
                return [DYYYFLEXSearchToken string:token options:options | TBWildcardOptionsSuffix];
            } else {
                // 只有句点可以跟在正斜杠后面
                @throw NSInternalInconsistencyException;
            }
    ...
    return [DYYYFLEXSearchToken string:token options:options];
}

+ (DYYYFLEXSearchToken *)scanMethodToken:(NSScanner *)scanner instance:(NSNumber **)instance {
    if (scanner.isAtEnd) {
    ...
    if (scanner.isAtEnd) {
        return [DYYYFLEXSearchToken string:@"" options:TBWildcardOptionsSuffix];
    }
