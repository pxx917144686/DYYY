//
//  DYYYFLEXKeyPathSearchController.m
//  FLEX
//
...
@interface DYYYFLEXKeyPathSearchController ()
...
/// 当搜索栏为空时为nil
@property (nonatomic) DYYYFLEXRuntimeKeyPath *keyPath;
...
@property (nonatomic) NSArray<NSArray<DYYYFLEXMethod *> *> *classesToMethods;
@end

@implementation DYYYFLEXKeyPathSearchController

+ (instancetype)delegate:(id<FLEXKeyPathSearchControllerDelegate>)delegate {
    DYYYFLEXKeyPathSearchController *controller = [self new];
    controller->_bundlesOrClasses = [DYYYFLEXRuntimeController allBundleNames];
...
- (void)setToolbar:(DYYYFLEXRuntimeBrowserToolbar *)toolbar {
    _toolbar = toolbar;
...
- (void)didSelectKeyPathOption:(NSString *)text {
    ...
    self.keyPath = [DYYYFLEXRuntimeKeyPathTokenizer tokenizeString:keyPath];
...
- (void)updateTable {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        if (self.classes) {
            ...
            // 我们想要显示这些类的特定方法
            // TODO: 以某种方式添加缓存
            NSMutableArray *methods = [DYYYFLEXRuntimeController
            ...
            DYYYFLEXRuntimeKeyPath *keyPath = self.keyPath;
            NSArray *models = [DYYYFLEXRuntimeController dataForKeyPath:keyPath];
            if (keyPath.methodKey) { // 我们正在查看方法
                self.bundlesOrClasses = nil;
                
                NSMutableArray *methods = models.mutableCopy;
                NSMutableArray<NSString *> *classes = [
                    DYYYFLEXRuntimeController classesForKeyPath:keyPath
                ];
...
- (void)setNonEmptyMethodLists:(NSMutableArray<NSArray<DYYYFLEXMethod *> *> *)methods
...
- (BOOL)searchBar:(UISearchBar *)searchBar shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    // 检查字符是否合法
    if (![DYYYFLEXRuntimeKeyPathTokenizer allowedInKeyPath:text]) {
        return NO;
    }
    ...
    @try {
        ...
        self.keyPath = [DYYYFLEXRuntimeKeyPathTokenizer tokenizeString:text];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    // ... 或者移除所有行
        ...
        _bundlesOrClasses = [DYYYFLEXRuntimeController allBundleNames];
...
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar {
    self.keyPath = DYYYFLEXRuntimeKeyPath.empty;
    [self updateTable];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 每个部分一行
        ...
        NSArray<DYYYFLEXMethod *> *methods = self.classesToMethods[indexPath.row];
        NSMutableString *summary = [NSMutableString new];
        [methods enumerateObjectsUsingBlock:^(DYYYFLEXMethod *method, NSUInteger idx, BOOL *stop) {
            NSString *format = nil;
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    NSString *bundleSuffixOrClass = self.bundlesOrClasses[indexPath.row];
    NSString *imagePath = [DYYYFLEXRuntimeController imagePathWithShortName:bundleSuffixOrClass];
