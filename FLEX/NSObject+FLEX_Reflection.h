...
@class DYYYFLEXMirror, DYYYFLEXMethod, DYYYFLEXIvar, DYYYFLEXProperty, DYYYFLEXMethodBase, DYYYFLEXPropertyAttributes, DYYYFLEXProtocol;
...
NSArray<DYYYFLEXProtocol *> * _Nullable FLEXGetConformedProtocols(_Nullable Class cls);

NSArray<DYYYFLEXIvar *> * _Nullable FLEXGetAllIvars(_Nullable Class cls);
/// @param cls 获取实例属性的类对象，
/// 或获取类属性的元类对象
NSArray<DYYYFLEXProperty *> * _Nullable FLEXGetAllProperties(_Nullable Class cls);
/// @param cls 获取实例方法的类对象，
/// 或获取类方法的元类对象
/// @param instance 用于标记方法是否为实例方法。
/// 不用于确定是获取实例方法还是类方法。
NSArray<DYYYFLEXMethod *> * _Nullable FLEXGetAllMethods(_Nullable Class cls, BOOL instance);
/// @param cls 获取所有实例和类方法的类对象。
NSArray<DYYYFLEXMethod *> * _Nullable FLEXGetAllInstanceAndClassMethods(_Nullable Class cls);
...
@property (nonatomic, readonly       ) DYYYFLEXMirror *flex_reflection;
@property (nonatomic, readonly, class) DYYYFLEXMirror *flex_reflection;
...
/// @return 这个类自身符合的协议列表。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXProtocol *> *flex_protocols;
...
/// @return \c DYYYFLEXMethod 对象的数组。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXMethod *> *flex_allMethods;
/// 接收类特有的所有实例方法。
/// @discussion 此方法只会检索接收类特有的方法。
/// 要检索父类上的实例变量，只需在 \c [self superclass] 上调用此方法。
/// @return \c DYYYFLEXMethod 对象的数组。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXMethod *> *flex_allInstanceMethods;
/// 接收类特有的所有类方法。
/// @discussion 此方法只会检索接收类特有的方法。
/// 要检索父类上的实例变量，只需在 \c [self superclass] 上调用此方法。
/// @return \c DYYYFLEXMethod 对象的数组。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXMethod *> *flex_allClassMethods;

/// 检索具有给定名称的类的实例方法。
/// @return 一个初始化的 \c DYYYFLEXMethod 对象，如果未找到方法，则返回 \c nil。
...
+ (IMP)replaceImplementationOfMethod:(DYYYFLEXMethodBase *)method with:(IMP)implementation useInstance:(BOOL)instanceMethod;
/// 交换给定方法的实现。
...
+ (void)swizzle:(DYYYFLEXMethodBase *)original with:(DYYYFLEXMethodBase *)other onInstance:(BOOL)instanceMethod;
/// 交换给定方法的实现。
/// @param instanceMethod YES 交换实例方法，NO 交换类方法。
...
/// @return \c DYYYFLEXIvar 对象的数组。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXIvar *> *flex_allIvars;

/// 检索具有相应名称的实例变量。
/// @return 一个初始化的 \c DYYYFLEXIvar 对象，如果未找到 Ivar，则返回 \c nil。
...
/// @discussion 如果你已经有一个 \c Ivar，这个方法比创建一个 \c DYYYFLEXIvar 并调用
/// \c -getIvarAddress: 更快。
/// @return 给定 ivar 在接收对象内存中的地址，
/// 如果找不到则返回 \c NULL。
...
/// @discussion 仅在目标实例变量是对象时使用。
/// 如果你已经有一个 \c Ivar，这个方法比创建一个 \c DYYYFLEXIvar 并调用
/// \c -setIvar: 更快。
...
/// 将接收对象上给定实例变量的值设置为
/// \e value 处的 \e size 字节数的数据。
/// @discussion 如果你已经有一个 \c Ivar，这比创建一个 \c DYYYFLEXIvar 并调用
...
/// 要检索父类上的实例变量，只需在 \c [self superclass] 上调用此方法。
/// @return \c DYYYFLEXProperty 对象的数组。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXProperty *> *flex_allProperties;
/// 接收类特有的所有实例属性。
/// @discussion 此方法只会检索接收类特有的属性。
/// 要检索父类上的实例变量，只需在 \c [self superclass] 上调用此方法。
/// @return \c DYYYFLEXProperty 对象的数组。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXProperty *> *flex_allInstanceProperties;
/// 接收类特有的所有类属性。
/// @discussion 此方法只会检索接收类特有的属性。
/// 要检索父类上的实例变量，只需在 \c [self superclass] 上调用此方法。
/// @return \c DYYYFLEXProperty 对象的数组。
@property (nonatomic, readonly, class) NSArray<DYYYFLEXProperty *> *flex_allClassProperties;

/// 检索具有给定名称的类的属性。
/// @return 一个初始化的 \c DYYYFLEXProperty 对象，如果未找到属性，则返回 \c nil。
