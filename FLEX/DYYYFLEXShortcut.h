...
/// 表示快捷方式部分中的一行。
///
/// 此协议的目的是允许将 \c DYYYFLEXShortcutsSection 
/// 的一小部分职责委托给另一个对象，用于单个任意行。
///
/// 创建自己的快捷方式并将它们附加/前置到
/// 类的现有快捷方式列表中非常有用。
@protocol DYYYFLEXShortcut <FLEXObjectExplorerItem>
...
@interface DYYYFLEXShortcut : NSObject <DYYYFLEXShortcut>

/// @param item 一个 \c NSString 或 \c FLEX* 元数据对象。
/// @note 您也可以传递一个符合 \c DYYYFLEXShortcut 的对象，
/// 这种情况下将返回该对象本身。
...
/// 提供 \c DYYYFLEXShortcut 协议的快速简单实现，
/// 允许您指定静态标题和其他所有内容的动态属性。
/// 传递给每个块的对象是传递给每个 \c DYYYFLEXShortcut 方法的对象。
///
/// 不支持 \c -editorWith: 方法。
@interface DYYYFLEXActionShortcut : NSObject <DYYYFLEXShortcut>
