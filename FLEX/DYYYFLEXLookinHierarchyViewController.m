...
@interface DYYYFLEXLookinHierarchyViewController () <UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, strong) UITableView *hierarchyTableView;
@property (nonatomic, strong) UIView *detailPanel;
@property (nonatomic, strong) UILabel *detailLabel;
@property (nonatomic, strong) UISegmentedControl *modeControl;
@property (nonatomic, strong) NSArray<DYYYFLEXLookinViewNode *> *flattenedHierarchy;
...
@implementation DYYYFLEXLookinHierarchyViewController
...
- (void)setupInspector {
    self.inspector = [DYYYFLEXLookinInspector sharedInstance];
...
- (void)compareSnapshot:(UIButton *)button {
    ...
    DYYYFLEXLookinComparisonViewController *comparisonVC = [[DYYYFLEXLookinComparisonViewController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DYYYFLEXLookinViewNode *node = self.flattenedHierarchy[indexPath.row];
...
- (void)showDetailsForNode:(DYYYFLEXLookinViewNode *)node {
...
- (void)lookinInspector:(DYYYFLEXLookinInspector *)inspector didSelectView:(UIView *)view {
    [self refreshHierarchy];
    
    // 查找对应的node并显示详情
    for (DYYYFLEXLookinViewNode *node in self.flattenedHierarchy) {
        if (node.view == view) {
            [self showDetailsForNode:node];
            break;
        }
...
- (void)lookinInspector:(DYYYFLEXLookinInspector *)inspector didUpdateHierarchy:(NSArray<DYYYFLEXLookinViewNode *> *)hierarchy {
