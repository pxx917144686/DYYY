...
@interface DYYYFLEXDoKitLogFilterViewController () <UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) UISegmentedControl *levelControl;
@property (nonatomic, strong) UISearchBar *searchBar;
@property (nonatomic, strong) NSArray<DYYYFLEXDoKitLogEntry *> *filteredLogs;
@end

@implementation DYYYFLEXDoKitLogFilterViewController
...
- (void)loadLogs {
    DYYYFLEXDoKitLogViewer *logViewer = [DYYYFLEXDoKitLogViewer sharedInstance];
...
- (void)applyFilters {
    DYYYFLEXDoKitLogViewer *logViewer = [DYYYFLEXDoKitLogViewer sharedInstance];
    NSArray<DYYYFLEXDoKitLogEntry *> *allLogs = logViewer.logEntries;  // ✅ 正确的类型
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DYYYFLEXDoKitLogEntry *logEntry = self.filteredLogs[indexPath.row];
    
    // 类型安全检查
    if (![logEntry isKindOfClass:[DYYYFLEXDoKitLogEntry class]]) {
        cell.textLabel.text = @"⚠️ 数据类型错误";
        cell.textLabel.textColor = FLEXSystemRedColor;  // ✅ 现在已定义
        cell.detailTextLabel.text = [NSString stringWithFormat:@"预期: DYYYFLEXDoKitLogEntry, 实际: %@", 
