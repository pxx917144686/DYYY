...
@implementation DYYYFLEXRuntimeUtility
...
+ (id)potentiallyUnwrapBoxedPointer:(id)returnedObjectOrNil type:(const FLEXTypeEncoding *)returnType {
    ...
    if (!returnsObjectOrClass) {
        ...
        if (returnsCString) {
            ...
            // 将伪装为void*的有效对象转换为id
            if ([DYYYFLEXRuntimeUtility pointerIsValidObjcObject:value.pointerValue]) {
...
+ (NSArray<DYYYFLEXObjectRef *> *)subclassesOfClassWithName:(NSString *)className {
    NSArray<Class> *classes = FLEXGetAllSubclasses(NSClassFromString(className), NO);
    NSArray<DYYYFLEXObjectRef *> *references = [DYYYFLEXObjectRef referencingClasses:classes];
    return references;
}
...
+ (id)performSelector:(SEL)selector
    ...
    NSMethodSignature *methodSignature = [DYYYFLEXMethod selector:selector class:cls].signature;
    if (!methodSignature) {
        // 不支持的类型编码
        return nil;
    }
...
+ (NSString *)readableTypeForEncoding:(NSString *)encodingString {
    ...
    const NSUInteger fieldNameOffset = [DYYYFLEXRuntimeUtility fieldNameOffsetForTypeEncoding:encodingCString];
...
+ (NSValue *)valueForPrimitivePointer:(void *)pointer objCType:(const char *)type {
    ...
    const NSUInteger fieldNameOffset = [DYYYFLEXRuntimeUtility fieldNameOffsetForTypeEncoding:type];
    if (fieldNameOffset > 0) {
