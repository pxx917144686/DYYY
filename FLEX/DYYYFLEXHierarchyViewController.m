//
//  DYYYFLEXHierarchyViewController.m
//  FLEX
//
...
@interface DYYYFLEXHierarchyViewController ()
@property (nonatomic, readonly, weak) id<FLEXHierarchyDelegate> hierarchyDelegate;
@property (nonatomic, readonly) DYYYFHSViewController *snapshotViewController;
@property (nonatomic, readonly) DYYYFLEXHierarchyTableViewController *treeViewController;
...
@implementation DYYYFLEXHierarchyViewController
...
- (id)initWithDelegate:(id)delegate viewsAtTap:(NSArray<UIView *> *)viewsAtTap selectedView:(UIView *)view {
    self = [super init];
    if (self) {
        NSArray<UIWindow *> *allWindows = DYYYFLEXUtility.allWindows;
        _hierarchyDelegate = delegate;
        _treeViewController = [DYYYFLEXHierarchyTableViewController
            windows:allWindows viewsAtTap:viewsAtTap selectedView:view
        ];

        if (viewsAtTap) {
            _snapshotViewController = [DYYYFHSViewController snapshotViewsAtTap:viewsAtTap selectedView:view];
        } else {
            _snapshotViewController = [DYYYFHSViewController snapshotWindows:allWindows];
        }
...
- (void)viewDidLoad {
    ...
    self.treeViewController.navigationItem.leftBarButtonItem = [UIBarButtonItem
        flex_itemWithImage:DYYYFLEXResources.toggle3DIcon target:self action:@selector(toggleHierarchyMode)
    ];
...
- (void)donePressed {
    ...
    // DYYYFLEXNavigationController doesn't ever close tabs itself 
    [DYYYFLEXTabList.sharedList closeTab:self];
