...
@class DYYYFLEXProperty, DYYYFLEXMethodDescription;

NS_ASSUME_NONNULL_BEGIN

#pragma mark DYYYFLEXProtocol
@interface DYYYFLEXProtocol : NSObject

/// 运行时注册的所有协议。
+ (NSArray<DYYYFLEXProtocol *> *)allProtocols;
...
@property (nonatomic, readonly) NSArray<DYYYFLEXMethodDescription *> *requiredMethods;
/// 协议的可选方法（如果有）。这包括属性的getter和setter方法。
@property (nonatomic, readonly) NSArray<DYYYFLEXMethodDescription *> *optionalMethods;
/// 此协议遵循的所有协议（如果有）。
...
@property (nonatomic, readonly, nullable) NSArray<DYYYFLEXProperty *> *properties API_DEPRECATED("使用下面更具体的访问器", ios(2.0, 10.0));

/// 协议中的必需属性（如果有）。
@property (nonatomic, readonly) NSArray<DYYYFLEXProperty *> *requiredProperties API_AVAILABLE(ios(10.0));
/// 协议中的可选属性（如果有）。
@property (nonatomic, readonly) NSArray<DYYYFLEXProperty *> *optionalProperties API_AVAILABLE(ios(10.0));

/// 内部使用
@property (nonatomic) id tag;

/// 不要与 \c -conformsToProtocol: 混淆，后者指的是当前
/// \c DYYYFLEXProtocol 实例，而不是底层的 \c Protocol 对象。
...
#pragma mark 方法描述
@interface DYYYFLEXMethodDescription : NSObject
