...
@interface DYYYFLEXLookinPreviewController ()
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) UIView *containerView;
@property (nonatomic, strong) NSArray<DYYYFLEXLookinDisplayItem *> *displayItems;
@property (nonatomic, strong) UISegmentedControl *dimensionControl;
@property (nonatomic, strong) UISlider *scaleSlider;
@end

@implementation DYYYFLEXLookinPreviewController
...
- (void)buildDisplayItemsFromView:(UIView *)view items:(NSMutableArray *)items depth:(NSInteger)depth {
    DYYYFLEXLookinDisplayItem *item = [[DYYYFLEXLookinDisplayItem alloc] init];
    item.view = view;
...
- (void)renderWithDisplayItems:(NSArray<DYYYFLEXLookinDisplayItem *> *)items {
    // 清除之前的渲染
...
- (void)render3DHierarchy:(NSArray<DYYYFLEXLookinDisplayItem *> *)items {
    CGFloat zOffset = 0;
    
    for (DYYYFLEXLookinDisplayItem *item in items) {
...
- (void)renderItem:(DYYYFLEXLookinDisplayItem *)item atZOffset:(CGFloat)zOffset {
    if (!item.view) return;
...
- (void)renderChildItems:(NSArray<DYYYFLEXLookinDisplayItem *> *)children parentZOffset:(CGFloat)parentZOffset {
    CGFloat childZOffset = parentZOffset;
    
    for (DYYYFLEXLookinDisplayItem *child in children) {
