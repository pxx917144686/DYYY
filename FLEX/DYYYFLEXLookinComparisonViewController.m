...
@interface DYYYFLEXLookinComparisonViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation DYYYFLEXLookinComparisonViewController
...
- (void)performComparison {
    ...
    NSArray<DYYYFLEXLookinViewNode *> *baseSnapshot = self.snapshots[baseIndex];
    NSArray<DYYYFLEXLookinViewNode *> *currentSnapshot = self.snapshots[selectedIndex];
    
    NSMutableArray *results = [NSMutableArray array];
    
    // 找出新增的视图
    for (DYYYFLEXLookinViewNode *currentNode in currentSnapshot) {
        BOOL found = NO;
        for (DYYYFLEXLookinViewNode *baseNode in baseSnapshot) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DYYYFLEXLookinViewNode *node = result[@"node"];
    ...
    if ([type isEqualToString:@"新增"]) {
        ...
        DYYYFLEXLookinViewNode *baseNode = result[@"baseNode"];
