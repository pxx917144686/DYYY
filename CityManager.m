...
// 替换原有的 convertWebpToGifSafely: 方法为 Untitled-1 里的完整实现
+ (void)convertWebpToGifSafely:(NSURL *)webpURL completion:(void (^)(NSURL *gifURL, BOOL success))completion {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (image.images && image.images.count > 1) {
            // 动画 WebP 逐帧解码
            CGImageDestinationRef destination = CGImageDestinationCreateWithURL((__bridge CFURLRef)gifURL, kUTTypeGIF, image.images.count, NULL);
            if (!destination) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (completion) {
                        completion(nil, NO);
            ...
            for (UIImage *frame in image.images) {
                NSDictionary *frameProperties = @{
                    (__bridge NSString *)kCGImagePropertyGIFDictionary: @{
                        (__bridge NSString *)kCGImagePropertyGIFDelayTime: @(image.duration / image.images.count),
                    }
                };
...
// 替换原有的 convertHeicToGif: 方法为 Untitled-1 里的完整实现
