...
static NSUserDefaults* getCorrectUserDefaults() {
...
static void loadCustomSocialStats() {
    NSUserDefaults *defaults = getCorrectUserDefaults();
...
// 创建个人主页数据编辑界面
void showSocialStatsEditAlert(UIViewController *viewController) {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"个人主页数据编辑"
                                                                   message:@"请输入要显示的自定义数据"
                                                            preferredStyle:UIAlertControllerStyleAlert];
    
    // 获取当前保存的数据
    NSUserDefaults *defaults = getCorrectUserDefaults();
    NSString *currentFollowers = [defaults objectForKey:DYYY_SOCIAL_FOLLOWERS_KEY] ?: @"";
    NSString *currentLikes = [defaults objectForKey:DYYY_SOCIAL_LIKES_KEY] ?: @"";
    NSString *currentFollowing = [defaults objectForKey:DYYY_SOCIAL_FOLLOWING_KEY] ?: @"";
    NSString *currentMutual = [defaults objectForKey:DYYY_SOCIAL_MUTUAL_KEY] ?: @"";
    
    // 添加粉丝数输入框
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.placeholder = @"粉丝数";
        textField.text = currentFollowers;
        textField.keyboardType = UIKeyboardTypeNumberPad;
    }];
    
    // 添加获赞数输入框
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.placeholder = @"获赞数";
        textField.text = currentLikes;
        textField.keyboardType = UIKeyboardTypeNumberPad;
    }];
    
    // 添加关注数输入框
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.placeholder = @"关注数";
        textField.text = currentFollowing;
        textField.keyboardType = UIKeyboardTypeNumberPad;
    }];
    
    // 添加互关数输入框
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.placeholder = @"互关数";
        textField.text = currentMutual;
        textField.keyboardType = UIKeyboardTypeNumberPad;
    }];
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"保存" 
        ...
        NSString *followers = ((UITextField *)textFields[0]).text;
        NSString *likes = ((UITextField *)textFields[1]).text;
        NSString *following = ((UITextField *)textFields[2]).text;
        NSString *mutual = ((UITextField *)textFields[3]).text;
        
        // 保存到UserDefaults
        NSUserDefaults *defaults = getCorrectUserDefaults();
        if (followers.length > 0) {
            [defaults setObject:followers forKey:DYYY_SOCIAL_FOLLOWERS_KEY];
        } else {
            [defaults removeObjectForKey:DYYY_SOCIAL_FOLLOWERS_KEY];
        }
        
        if (likes.length > 0) {
            [defaults setObject:likes forKey:DYYY_SOCIAL_LIKES_KEY];
        } else {
            [defaults removeObjectForKey:DYYY_SOCIAL_LIKES_KEY];
        }
        
        if (following.length > 0) {
            [defaults setObject:following forKey:DYYY_SOCIAL_FOLLOWING_KEY];
        } else {
            [defaults removeObjectForKey:DYYY_SOCIAL_FOLLOWING_KEY];
        }
        
        if (mutual.length > 0) {
            [defaults setObject:mutual forKey:DYYY_SOCIAL_MUTUAL_KEY];
        } else {
            [defaults removeObjectForKey:DYYY_SOCIAL_MUTUAL_KEY];
        }
        ...
        [viewController presentViewController:successAlert animated:YES completion:nil];
    }]];
