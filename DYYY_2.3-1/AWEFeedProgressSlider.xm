...
@interface AWEFakeProgressSliderView ()
- (void)applyCustomProgressStyle;
@end
...
- (void)handleSettingChanged:(NSNotification *)notification {
    ...
    @try {
        ...
        [key isEqualToString:@"DYYYShowScheduleDisplay"]) {
...
- (void)setLimitUpperActionArea:(BOOL)arg1 {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (isShowSchedule) {
            ...
            BOOL showHorizontal = [scheduleStyle isEqualToString:@"进度条水平显示"];
            ...
            if (showLeftRemainingTime || showLeftCompleteTime || showBothSides || showHorizontal) {
            ...
            if (showRemainingTime || showCompleteTime || showBothSides || showHorizontal) {
            ...
            // 水平显示特殊处理：将标签放在进度条的左右两端，水平排列
            if (showHorizontal) {
                UILabel *leftLabel = [parentView viewWithTag:10001];
                UILabel *rightLabel = [parentView viewWithTag:10002];
                if (leftLabel && rightLabel) {
                    // 左侧标签放在进度条左端
                    leftLabel.frame = CGRectMake(sliderFrame.origin.x, labelY, leftLabel.frame.size.width, labelHeight);
                    // 右侧标签放在进度条右端
                    CGFloat rightLabelX = CGRectGetMaxX(sliderFrame) - rightLabel.frame.size.width;
                    rightLabel.frame = CGRectMake(rightLabelX, labelY, rightLabel.frame.size.width, labelHeight);
                }
            }
        }
        [self setNeedsLayout];
