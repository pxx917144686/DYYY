...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message {
    if (self = [super initWithFrame:UIScreen.mainScreen.bounds]) {
        ...
        // 添加许可证相关链接支持
        NSRange unlicenseRange = [message rangeOfString:@"Unlicense许可证 unlicense.org"];
        if (unlicenseRange.location != NSNotFound) {
            [attributedString addAttribute:NSLinkAttributeName value:@"https://unlicense.org/#unlicensed-free-software" range:unlicenseRange];
        }

        NSRange hackerCultureRange = [message rangeOfString:@"黑客文化历史 en.wikipedia.org/wiki/Hacker_culture"];
        if (hackerCultureRange.location != NSNotFound) {
            [attributedString addAttribute:NSLinkAttributeName value:@"https://en.wikipedia.org/wiki/Hacker_culture" range:hackerCultureRange];
        }

        NSRange unixHistoryRange = [message rangeOfString:@"UNIX历史 en.wikipedia.org/wiki/History_of_Unix"];
        if (unixHistoryRange.location != NSNotFound) {
            [attributedString addAttribute:NSLinkAttributeName value:@"https://en.wikipedia.org/wiki/History_of_Unix" range:unixHistoryRange];
        }
        
        // 添加对简单@pxx917144686文本的链接支持
        NSRange simplePxxRange = [message rangeOfString:@"@pxx917144686"];
        if (simplePxxRange.location != NSNotFound && githubRange.location == NSNotFound) {
            [attributedString addAttribute:NSLinkAttributeName value:@"https://github.com/pxx917144686" range:simplePxxRange];
        }
        
        // 添加对@维他入我心地址的链接支持（确保兼容性）
        NSRange vitaGithubRange = [message rangeOfString:@"@维他入我心 Wtrwx/DYYY"];
        if (vitaGithubRange.location != NSNotFound) {
            [attributedString addAttribute:NSLinkAttributeName value:@"https://github.com/Wtrwx/DYYY" range:vitaGithubRange];
        }
