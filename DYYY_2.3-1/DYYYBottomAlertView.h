...
/**
 * 显示警告框，使用旧版本的方法签名（兼容性）
 * @param title 标题文本
 * @param message 消息文本
 * @param confirmButton 确认按钮文本
 * @param cancelButton 取消按钮文本
 * @param confirmBlock 点击确认按钮回调
 * @param cancelBlock 点击取消按钮回调
 * @return 返回控制器实例，可用于视图管理
 */
+ (UIViewController *)showAlertWithTitle:(nullable NSString *)title
                                 message:(nullable NSString *)message
                           confirmButton:(nullable NSString *)confirmButton
                            cancelButton:(nullable NSString *)cancelButton
                            confirmBlock:(nullable DYYYAlertActionHandler)confirmBlock
                             cancelBlock:(nullable DYYYAlertActionHandler)cancelBlock;
