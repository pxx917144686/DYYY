...
@interface AWEElementStackView ()
- (UIViewController *)firstAvailableUIViewController;
@end

@interface UIView (DYYYExtensions)
- (UIViewController *)firstAvailableUIViewController;
@end
...
#pragma clang diagnostic pop
        ...
        // 底部栏完整高度 = 底部栏本身高度(49) + 安全区域高度
        CGFloat tabBarBaseHeight = 49.0; // 标准底部栏高度
        cachedHeight = tabBarBaseHeight + safeAreaBottom;
        
        // 如果没有安全区域，使用默认高度
        if (cachedHeight < tabBarBaseHeight) {
            cachedHeight = tabBarBaseHeight;
        }
