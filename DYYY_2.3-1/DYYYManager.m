...
+ (void)downloadMedia:(NSURL *)url mediaType:(MediaType)mediaType completion:(void (^)(BOOL success))completion {
    [self downloadMedia:url mediaType:mediaType audio:nil completion:completion];
}
...
+ (UIColor *)colorWithHexString:(NSString *)hexString {
  ...
  if ([hexString containsString:@","]) {
    ...
    if (colors.count >= 2) {
      UIColor *color1 = [self colorWithHexString:colors[0]];
      UIColor *color2 = [self colorWithHexString:colors[1]];
      // 返回第一个颜色
      return color1;
    }
  }
  ...
  if (hexString.length == 8) {
    ...
    NSScanner *scanner = [NSScanner
        scannerWithString:[hexString substringWithRange:NSMakeRange(0, 2)]];
    unsigned int alphaValue;
    [scanner scanHexInt:&alphaValue];
    alpha = (CGFloat)alphaValue / 255.0;

    scanner = [NSScanner
        scannerWithString:[hexString substringWithRange:NSMakeRange(2, 2)]];
    unsigned int redValue;
    [scanner scanHexInt:&redValue];
    red = (CGFloat)redValue / 255.0;

    scanner = [NSScanner
        scannerWithString:[hexString substringWithRange:NSMakeRange(4, 2)]];
    unsigned int greenValue;
    [scanner scanHexInt:&greenValue];
    green = (CGFloat)greenValue / 255.0;

    scanner = [NSScanner
        scannerWithString:[hexString substringWithRange:NSMakeRange(6, 2)]];
